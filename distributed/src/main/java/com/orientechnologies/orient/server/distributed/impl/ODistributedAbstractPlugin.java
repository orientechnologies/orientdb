...
public abstract class ODistributedAbstractPlugin extends OServerPluginAbstract
  ...
  public ODistributedResponse sendRequest(
    ...
    if (db == null) {
      ODistributedServerLog.error(
          ...
          null,
          DIRECTION.OUT,
          "Distributed database '%s' not found",
          iDatabaseName);
      throw new ODistributedException(
          "Distributed database '" + iDatabaseName + "' not found on server '" + nodeName + "'");
    }
  ...
  protected void backupCurrentDatabase(final String iDatabaseName) {
    ...
    if (backupDirectory.startsWith("/")) backupPath = backupDirectory;
      ...
      } else backupPath = serverInstance.getDatabaseDirectory() + backupDirectory;
    }
    ...
    if (oldDirectory.exists() && oldDirectory.isDirectory()) {
      try {
        Files.move(
            oldDirectory.toPath(), backupfullpath.toPath(), StandardCopyOption.REPLACE_EXISTING);
      } catch (IOException e) {
        ODistributedServerLog.warn(
            this,
            nodeName,
