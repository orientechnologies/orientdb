...
public class OHazelcastClusterMetadataManager
  ...
  protected void waitStartupIsCompleted(CountDownLatch serverStarted) throws InterruptedException {
    ...
    try {
      ...
      if (key.startsWith(CONFIG_NODE_PREFIX)) {
        ...
        if (!iEvent.getMember().equals(hazelcastInstance.getCluster().getLocalMember())
          ...
          final ODistributedServerManager.DB_STATUS s =
              getDatabaseStatus(getLocalNodeName(), databaseName);
    ...
    if (activeNodes.putIfAbsent(joinedNodeName, member) == null) {
      ...
      for (String db : messageService.getDatabases()) {
        if (getDatabaseConfiguration(db).isAutoDeploy()
            && getDatabaseStatus(joinedNodeName, db)
