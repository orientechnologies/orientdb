    ...
    if (nodes.isEmpty()) {
      switch (localResult.getResponseType()) {
        ...
        case OTxRecordLockTimeout.ID:
          {
            int timeout =
                database.getConfiguration().getValueAsInteger(DISTRIBUTED_ATOMIC_LOCK_TIMEOUT);
            throw new ODistributedRecordLockedException(
                dManager.getLocalNodeName(),
                ((OTxRecordLockTimeout) localResult).getLockedId(),
                timeout);
          }
      ...
      switch (resultPayload.getResponseType()) {
        case OTxSuccess.ID:
          ...
          sendPhase2Task(
              involvedClusters,
              nodes,
              new OTransactionPhase2Task(requestId, true, txTask.getRids(), getLsn()));
          localOk(requestId, database);
          break;
        case OTxException.ID:
          // Exception send ko and throws the exception
          sendPhase2Task(
              involvedClusters,
              nodes,
              new OTransactionPhase2Task(requestId, false, txTask.getRids(), getLsn()));
          localKo(requestId, database);
