...
public class ODistributedResponseManager {
  ...
  public ODistributedResponseManager(final OHazelcastPlugin iManager, final ODistributedRequest iRequest,
  ...
  public boolean collectResponse(final ODistributedResponse response) {
    ...
    synchronized (responseLock) {
      ...
      if (responsePayload instanceof ODiscardedResponse)
        discardedResponses++;
  ...
  public ODistributedResponse getFinalResponse() {
    synchronized (responseLock) {
      ...
      case UNION: {
        ...
        final HashMap<String, Object> payloads = new HashMap<String, Object>();
        for (Map.Entry<String, Object> entry : responses.entrySet())
          if (entry.getValue() != NO_RESPONSE)
  ...
  private boolean realignRecordClusters() {
    ...
    for (ODistributedResponse r : getReceivedResponses()) {
      ...
      if (task instanceof OCreateRecordTask) {
        Object badResponse = r.getPayload();
        if (badResponse instanceof OSerializableWrappedResponse)
          badResponse = ((OSerializableWrappedResponse) badResponse).getWrapped();
