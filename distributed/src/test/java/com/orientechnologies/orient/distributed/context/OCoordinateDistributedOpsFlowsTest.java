...
public class OCoordinateDistributedOpsFlowsTest {
  ...
  public void testRemoveDatabaseMember() {
    ...
    var version =
        flow.getContexts().get(node1).getOps().getDatabaseTopology().getDatabaseVersion(dbId);
    var result = flow.execute(new ORemoveDatabaseMembers(dbId, List.of(node3), version.next()));
  ...
  public void testRemoveTopologyMember() {
    ...
    var result = flow.execute(new ORemoveTopologyMember(node3, version.next()));
  ...
  public void testSetDatabaseQuorum() {
    ...
    var version =
        flow.getContexts().get(node1).getOps().getDatabaseTopology().getDatabaseVersion(dbId);
    var result = flow.execute(new OSetDatabaseQuorum(dbId, 3, version.next()));
