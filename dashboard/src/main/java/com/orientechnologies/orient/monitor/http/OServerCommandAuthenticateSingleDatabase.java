...
public class OServerCommandAuthenticateSingleDatabase extends OServerCommandAuthenticatedDbAbstract {
    ...
    public OServerCommandAuthenticateSingleDatabase(final OServerCommandConfiguration iConfiguration) {
    }
    ...
    protected boolean authenticate(final OHttpRequest iRequest, final OHttpResponse iResponse,
        ...
        try {
            ...
            OMonitorPlugin monitorPlugin = OServerMain.server().getPluginByClass(OMonitorPlugin.class);
            ...
            try {
                if (urlParts.length == 2) {
                    ...
                    if (authorizedServer){
                        db = new ODatabaseDocumentTx("remote:" + host + ":" + port + "/" + iDatabaseName).open(iAuthenticationParts.get(0), iAuthenticationParts.get(1));
                    }else{
                ...
                // WRONG USER/PASSWD
                OLogManager.instance().error(this, "Cannot login to the database '" + iDatabaseName + "', the server is not under monitoring", OSecurityAccessException.class, e);
