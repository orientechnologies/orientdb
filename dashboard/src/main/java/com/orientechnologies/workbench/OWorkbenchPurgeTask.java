...
public final class OWorkbenchPurgeTask extends TimerTask {

	private final OWorkbenchPlugin handler;

	OWorkbenchPurgeTask(final OWorkbenchPlugin iHandler) {
		...
		try {
			...
			if (response.size() > 0) {
				...
				if (deleteConfiguration != null) {
					...
					if (hours != null) {
						OWorkbenchPurgeMetricLogHelper.deleteMetrics(hours,
								this.handler.getDb());
					}
				}
				...
				if(notificationsConfiguration!= null){
					...
					if (hours != null) {
						OWorkbenchPurgeMetricLogHelper.deleteLogs(hours,
								this.handler.getDb());
					}
				}
			}
