...
OForm.prototype.formtoobject = function(prefix) {
    ...
    var cursor = this.findRoot(this.object, prefix.substring(form.getBasePrefix().length));
...
OForm.prototype.values = function(prefix) {
    ...
    objects.each( function (i, el) {
        ...
        if (id && id.indexOf(prefix + "_") == 0 && id.indexOf("!") < 0 && $(el).is(":visible")) {
            var path = id.split('_');
            path.shift();
            var pointer = null;

            $.each(path, function(i, stringItem) {
                if (!pointer) {
                    pointer = driver;
                }
                ...
                if (i == path.length-1) {
                    pointer[stringItem] = el.value;
                } else {
                    pointer = pointer[stringItem];
                    var verify = path[i+1];

                    if (!parseInt(verify) && verify != 0) {
                        pointer['@type'] = 'd';
                        if(!pointer['@class']) {
...
OForm.prototype.findClassInObject = function(id, i) {
    form = this;
    var path = id.substring().split('_');

    if (form.object) {
        pointer = form.object;
        for(s=0; s<=i; s++) {
            var go = 0;
            if (!parseInt(path[s]) && path[s] != 0) {
                go = path[s];
            }
            pointer = pointer[go];
        }
    ...
    if (component != null) {
        ...
        for (index in array) {
            ...
            if (value != null && typeof value == "object") {
                ...
                this.object2form(value, prefix + "_" + index, prefix
                + "_?0", level + 1);
            } else {
                if (index == 0)
