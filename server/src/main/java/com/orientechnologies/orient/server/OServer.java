...
public class OServer {
    ...
    if (serverSecurity != null && serverSecurity.isEnabled()) {
      ...
      if (user != null && user.password != null) {
        if (OSecurityManager.instance().checkPassword(iPassword, user.password)
            && isAllowed(iUserName, iResourceToCheck)) {
          return user;
        }
      }
    }
  ...
  public void addUser(final String iName, String iPassword, final String iPermissions)
      throws IOException {
    if (iPassword == null) {
  ...
  private List<ServerPluginConfig> loadServerPlugins() throws ClassNotFoundException {
    ...
    if (configuration.handlers != null) {
      for (OServerHandlerConfiguration h : configuration.handlers) {
        ...
        Class<?> pluginClass = loadClass(h.clazz);
  ...
  protected void registerPlugins(List<ServerPluginConfig> serverPlugins)
