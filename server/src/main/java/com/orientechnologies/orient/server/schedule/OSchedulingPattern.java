...
public class OSchedulingPattern {
	...
	public OSchedulingPattern(String pattern) throws OInvalidPatternException {
		...
		while (st1.hasMoreTokens()) {
			...
			try {
				...
				throw new OInvalidPatternException("invalid pattern \""
	 ...
	 */
	private OValueMatcher buildValueMatcher(String str, ValueParser parser)
			throws Exception {
	...
	public boolean match(TimeZone timezone, long millis) {
		...
		for (int i = 0; i < matcherSize; i++) {
			...
			boolean eval = minuteMatcher.match(minute)
					...
					&& ((dayOfMonthMatcher instanceof ODayOfMonthValueMatcher) ? ((ODayOfMonthValueMatcher) dayOfMonthMatcher)
