...
//    public static final String DEFAULT_CERTIFICATE_OWNER = "CN=C02WP0WUHV2Q.local";
    ...
    public void setCertificate_SN(long certificate_SN) throws SwitchToDefaultParamsException{
        if (certificate_SN <= 11){
            ...
            throw new SwitchToDefaultParamsException("the value "+certificate_SN+ " culd not be used as a Certificate Serial Nuber, the value will be set to:"+sn);
        }else
          this.certificate_SN = BigInteger.valueOf(certificate_SN);
    }
     ...
     * (algorithm-specific) values of the generated keys.Our People
     *
     * <p>This method will computes and returns a new key pair every time it is called.
     *
     * @throws NoSuchAlgorithmException if the algorithm String not match with the supported key generation schemes.
     *
     ...
     * {@link #computeKeyPair computeKeyPair(this.algorithm,this.key_size)}. It uses the value pair (DEFAULT_CERTIFICATE_ALGORITHM,DEFAULT_CERTIFICATE_KEY_SIZE) if the setted fields are not valid.
     *
     * @throws NoSuchAlgorithmException if the algorithm String not match with the supported key generation schemes.
     **/
    ...
    public void composeSelfSignedCertificate(){
        try {
            this.certificate = generateSelfSignedCertificate(this.key_pair,this.validity,this.owner_FDN,this.certificate_SN);
    ...
    public static X509Certificate generateSelfSignedCertificate(KeyPair key_pair, int validity, String owner_FDN, BigInteger cert_SN) throws CertificateException, IOException {
    ...
    // set certificate VERSION
        ...
        // set certificate Signature ALGORITHM = RSA
//        info.set(X509CertInfo.ALGORITHM_ID,new CertificateAlgorithmId(new AlgorithmId(AlgorithmId.sha256WithDSA_oid)));
...
// original signature    static String[] generate(String fqdn, KeyPair keypair, SecureRandom random, Date notBefore, Date notAfter)
//    // Snippet sample copied by codata.com
//    static void generate(String fqdn, KeyPair keypair, SecureRandom random, Date notBefore, Date notAfter)
//            throws Exception {
...
//
//                String cert_content = "Owner: CN=OrientDB, OU=SAP HANA Core, O=SAP SE, L=Walldorf, C=DE\n" +
...
//                "         SHA256: 42:9C:B9:0F:AB:0A:58:AB:42:C3:DB:39:43:EA:61:B2:C8:F8:72:D0:8F:99:B4:95:81:21:2C:E0:96:3A:60:68\n" +
...
//    }
    ...
    public static boolean checkCertificate(X509Certificate cert, PublicKey public_key, Date date) throws NoSuchProviderException, CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        cert.checkValidity(date);
        cert.verify(public_key);
        return true;
    }

    public static boolean checkCertificate(X509Certificate cert, PublicKey public_key) throws NoSuchProviderException, CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
    ...
    public boolean checkThisCertificate() throws NoSuchAlgorithmException, CertificateException, NoSuchProviderException, InvalidKeyException, SignatureException {
        return checkCertificate(this.certificate,this.key_pair.getPublic(), new Date(System.currentTimeMillis()));
    }
