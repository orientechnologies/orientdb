...
public class OrientGraphMatchStrategyTest {
    ...
    public void shouldUseMatchOptimization() {
        ...
        try {
            ...
            GraphTraversal.Admin<Vertex, Map<String, Object>> admin = traversal.V()
                    .match(__.as("a").has("name", "Enrico").out("Friends").as("b")).asAdmin();
    ...
    public void shouldUseMatchOptimizationWithLabel() {
        ...
        try {
            ...
            GraphTraversal.Admin<Vertex, Map<String, Object>> admin = traversal.V()
                    .match(__.as("a").has("name", "Foo").out("Friends").as("b")).asAdmin();
            ...
            admin = traversal.V().match(__.as("a").hasLabel("Person").has("name", "Foo").out("Friends").as("b")).asAdmin();
    ...
    public void shouldFetchDataUsingMatchOptimization() {
        ...
        try {
            ...
            List<Map<String, Object>> admin = traversal.V().match(__.as("a").has("name", "Foo0").in("Friends").as("b")).toList();

            Assert.assertEquals(1, admin.size());

            admin = traversal.V().match(__.as("a").hasLabel("Person").has("name", "Foo0").in("Friends").as("b")).toList();
