...
public class OrientGraphStep<S, E extends Element> extends GraphStep<S, E> implements HasContainerHolder {
    ...
    private Iterator<? extends Vertex> vertices() {
        return elements(OrientGraph::vertices, OrientGraph::getIndexedVertices, OrientGraph::vertices, v -> v.getVertex().get());
    }

    private Iterator<? extends Edge> edges() {
        return elements(OrientGraph::edges, OrientGraph::getIndexedEdges, OrientGraph::edges, (e) -> e.getEdge().get());
    }
    ...
    private <ElementType extends Element> Iterator<? extends ElementType> elements(
            ...
            Function<OrientGraph, Iterator<ElementType>> getAllElements, Function<OGremlinResult, ElementType> getElement) {
    ...
    public Set<OrientIndexQuery> findIndex() {
        ...
        if (!classLabels.isEmpty()) {
            ...
            this.hasContainers.stream().filter(c -> indexedKeys.contains(c.getKey()) && (c.getPredicate().getBiPredicate() == Compare.eq
                    || c.getPredicate().getBiPredicate() == Contains.within)).findAny().ifPresent(requestedKeyValue -> {
    ...
    private Iterator<Object> getValueIterator(HasContainer c) {
        return c.getPredicate().getBiPredicate() == Contains.within ? ((Iterable<Object>) c.getValue()).iterator() : IteratorUtils.of(c.getValue());
    }
    ...
    public String toString() {
        if (this.hasContainers.isEmpty())
            ...
            return 0 == this.ids.length ? StringFactory.stepString(this, this.returnClass.getSimpleName().toLowerCase(), this.hasContainers) : StringFactory
