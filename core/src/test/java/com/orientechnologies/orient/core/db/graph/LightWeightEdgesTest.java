...
public class LightWeightEdgesTest {
  ...
  public void testCreateDeleteLightWeight() {
    ...
    session
        .command(
            "create edge `Edge` FROM (select from `Vertex` where name='aName' ) TO (select from `Vertex` where name='bName' )")
        .close();
    ...
    session
        .command(
            " delete edge `Edge` from (select from  `Vertex` where name='aName' ) to (select from  `Vertex` where name='bName' )")
        .close();
    try (OResultSet res = session.query("select out_Edge from `Vertex`")) {
      OResult next = res.next();
