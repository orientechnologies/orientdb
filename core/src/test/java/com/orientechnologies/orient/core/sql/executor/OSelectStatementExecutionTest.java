...
public class OSelectStatementExecutionTest extends BaseMemoryDatabase {
  ...
  @Test
  public void testSelectOrderByIndexNullDesc() {
    String className = "testSelectOrderByIndexNullDesc";
    OClass cl = db.getMetadata().getSchema().createClass(className);
    cl.createProperty("name", OType.STRING).createIndex(OClass.INDEX_TYPE.NOTUNIQUE);
    for (int i = 0; i < 30; i++) {
      ODocument doc = db.newInstance(className);
      if (i % 2 == 0) {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      if (thisName == null) {
        // Validate that no value come before null
        assertNull(lastName);
      } else if (lastName != null) {
        Assert.assertTrue(lastName.compareTo(thisName) >= 0);
      }
      lastName = thisName;
    }
    Assert.assertFalse(result.hasNext());
  ...
  @Test
  public void testSelectOrderByIndexNullAsc() {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      String thisName = item.getProperty("name");
      if (thisName == null) {
        // Validate that null come after the last
        assertEquals(lastName, "name9");
      } else {
        if (lastName != null) {
          Assert.assertTrue(lastName.compareTo(thisName) <= 0);
        }
  ...
  @Test
  public void testSelectOrderByxNullDesc() {
    String className = "testSelectOrderByxNullDesc";
    OClass cl = db.getMetadata().getSchema().createClass(className);
  ...
  @Test
  public void testSelectOrderByNullAsc() {
    String className = "testSelectOrderByNullAsc";
    OClass cl = db.getMetadata().getSchema().createClass(className);
    cl.createProperty("name", OType.STRING);
    for (int i = 0; i < 30; i++) {
      ODocument doc = db.newInstance(className);
    ...
    result =
        db.query(
            "SELECT NativeState, NativeState[0].Name AS _NativeState_Name \n"
                ...
                + " ORDER BY NativeState[0].Name DESC");
    printExecutionPlan(result);
    names = result.stream().map((x) -> (String) x.getProperty("_NativeState_Name")).toList();
    assertEquals(Arrays.asList("P", "M", "G"), names);
    result.close();
  }
  ...
  public void testFetchFromClustersNumberOrderByRidAsc() {
    ...
    for (int i = 0; i < 10; i++) {
      ODocument doc = db.newInstance(className);
      doc.setProperty("val", i);
      db.save(doc, targetClusterName2);
    }
  ...
  public void testFetchFromSingleRid3() {
    ...
    db.save(document, db.getClusterNameById(0));
  ...
  public void testFetchFromClassWithIndex() {
    ...
    Assert.assertFalse(p2.getIndexes().isEmpty());
    result.close();
  }
  ...
  public void testExpand1() {
    ...
    db.getMetadata().getSchema().createClass(childClassName);
    db.getMetadata().getSchema().createClass(parentClassName);
  ...
  public void testFetchFromSubclassIndexes3() {
    ...
    OClass parentClass = db.getMetadata().getSchema().createClass(parent);
    OClass childClass1 = db.getMetadata().getSchema().createClass(child1, parentClass);
    db.getMetadata().getSchema().createClass(child2, parentClass);
  ...
  public void testRidPagination1() {
    ...
    for (int i = 0; i < clusterIds.length; i++) {
      ...
      db.save(elem, db.getClusterNameById(clusterIds[i]));
    }
  ...
  public void testEmptyListNoIndex() {
    ...
    try (OResultSet result = db.query("select from " + className + " where noIndex = []")) {
      Assert.assertTrue(result.getExecutionPlan().get().getIndexes().isEmpty());
  ...
  public void testMapByValueIndex() {
    ...
    OClass clazz1 = db.createClassIfNotExist(className);
    clazz1.createProperty("themap", OType.EMBEDDEDMAP, OType.STRING);
  ...
  public void testOutJson() {
    ...
    db.command(
            "create edge e from (select from SimpleV where name='foo') to (select from SimpleV"
                + " where name='foo1')")
        .close();
    try (OResultSet rs =
        db.query(
            "select name, outs.tojson() as json from ( select name, out() as outs from SimpleV"
                + " where name='foo') ")) {
      Assert.assertTrue(rs.hasNext());
      OResult item = rs.next();
  ...
  public void testOutJsonEmb() {
    ...
    try (OResultSet rs =
        db.query(
            "select name, outs as json from ( select name, out().toJson() as outs from SimpleV"
                + " where name='foo') ")) {
      Assert.assertTrue(rs.hasNext());
      OResult item = rs.next();
      Assert.assertFalse(((String) item.getProperty("json")).contains("null"));
