...
public class ClusterPageAppendRecordPOTest {
  ...
  public void testRedo() {
    ...
    try {
      ...
      Assert.assertEquals(1, clusterPage.getRecordVersion(0));
      Assert.assertEquals(2, clusterPage.getRecordVersion(1));

      Assert.assertArrayEquals(new byte[] { 1 }, clusterPage.getRecordBinaryValue(0, 0, 1));
      Assert.assertArrayEquals(new byte[] { 2 }, clusterPage.getRecordBinaryValue(1, 0, 1));

      pageOperation.redo(restoredCacheEntry);

      Assert.assertEquals(3, clusterPage.getRecordsCount());

      Assert.assertEquals(1, clusterPage.getRecordVersion(0));
      Assert.assertEquals(2, clusterPage.getRecordVersion(1));
      Assert.assertEquals(3, clusterPage.getRecordVersion(2));

      Assert.assertArrayEquals(new byte[] { 1 }, clusterPage.getRecordBinaryValue(0, 0, 1));
      Assert.assertArrayEquals(new byte[] { 2 }, clusterPage.getRecordBinaryValue(1, 0, 1));
      Assert.assertArrayEquals(new byte[] { 3 }, clusterPage.getRecordBinaryValue(2, 0, 1));
