...
public class OSBTreeCollectionManagerSharedTest extends DatabaseAbstractTest {
  ...
  public void testEvictionAllReleased() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      OSBTreeBonsai<OIdentifiable, Integer> tree = atomicOperationsManager.calculateInsideAtomicOperation(null,
          atomicOperation -> sbTreeCollectionManager.createAndLoadTree(atomicOperation, clusterId));
  ...
  public void testEvictionTwoIsNotReleased() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      OSBTreeBonsai<OIdentifiable, Integer> tree = atomicOperationsManager.calculateInsideAtomicOperation(null,
          atomicOperation -> sbTreeCollectionManager.createAndLoadTree(atomicOperation, database.getDefaultClusterId()));
      createdTrees.add(tree);
      sbTreeCollectionManager.releaseSBTree(new OBonsaiCollectionPointer(tree.getFileId(), tree.getRootBucketPointer()));
    }
