...
public abstract class OLocalHashTableV3Base {
  ...
  public void testKeyDeleteRandomUniform() throws IOException {
    ...
    for (int key : keys) {
      if (key % 3 == 0)
        atomicOperationsManager.executeInsideAtomicOperation(null, atomicOperation -> localHashTable.remove(atomicOperation, key));
    }
    ...
    for (int i = 0; i < KEYS_COUNT; i++) {
      if (i % 3 == 0) {
        ...
        atomicOperationsManager.executeInsideAtomicOperation(null,
            atomicOperation -> Assert.assertEquals(localHashTable.remove(atomicOperation, key), String.valueOf(key)));
      }
    }
  ...
  public void testKeyAddDelete() throws IOException {
    ...
    for (int i = 0; i < KEYS_COUNT; i++) {
      ...
      if (i % 2 == 0) {
        final int key = i;
        atomicOperationsManager.executeInsideAtomicOperation(null,
            atomicOperation -> localHashTable.put(atomicOperation, KEYS_COUNT + key, String.valueOf(KEYS_COUNT + key)));
      }
    }
  ...
  public void testKeyPutRemoveNullKey() throws IOException {
    ...
    atomicOperationsManager
        .executeInsideAtomicOperation(null, atomicOperation -> localHashTable.put(atomicOperation, null, "null"));
    ...
    atomicOperationsManager.executeInsideAtomicOperation(null,
        atomicOperation -> Assert.assertEquals(localHashTable.remove(atomicOperation, null), "null"));

    for (int i = 0; i < 5; i++) {
      final int key = i;
      atomicOperationsManager
          .executeInsideAtomicOperation(null, atomicOperation -> Assert.assertNull(localHashTable.remove(atomicOperation, key)));
    }

    atomicOperationsManager
        .executeInsideAtomicOperation(null, atomicOperation -> Assert.assertNull(localHashTable.remove(atomicOperation, null)));
