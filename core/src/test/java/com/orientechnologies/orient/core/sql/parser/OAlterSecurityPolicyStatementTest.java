...
public class OAlterSecurityPolicyStatementTest extends OParserTestAbstract {
  ...
  public void testPlain() {
    ...
    checkRightSyntax("ALTER SECURITY POLICY foo SET CREATE = (name = 'foo'), READ = (name = 'foo')" +
            ", BEFORE UPDATE = (name = 'foo'), AFTER UPDATE = (name = 'foo'), DELETE = (name = 'foo'), EXECUTE = (name = 'foo')");

    checkRightSyntax("ALTER SECURITY POLICY foo REMOVE CREATE, READ, BEFORE UPDATE, AFTER UPDATE, DELETE, EXECUTE");
