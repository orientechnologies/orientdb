...
public class ReadWriteDiskCacheTest {
  ...
  public void testAddFourItems() throws IOException {
    ...
    for (int i = 0; i < 4; i++) {
      entries[i] = readBuffer.load(fileId, i, false, writeBuffer, 1, true, null);
      if (entries[i] == null) {
    ...
    for (int i = 0; i < 4; i++) {
      OCacheEntry entry = generateEntry(fileId, i, entries[i].getCachePointer().getBufferDuplicate(), bufferPool, false,
  ...
  public void testFrequentlyReadItemsAreMovedInAm() throws Exception {
    ...
    final OByteBufferPool bufferPool = OByteBufferPool.instance();
    for (int i = 2; i < 4; i++) {
      OCacheEntry lruEntry = generateEntry(fileId, i, entries[i].getCachePointer().getBufferDuplicate(), bufferPool, false,
  ...
  public void testPrefetchPagesInA1inQueue() throws Exception {
    ...
    OCacheEntry cacheEntry = readBuffer.load(fileId, 0, false, writeBuffer, 1, true, null);
    readBuffer.release(cacheEntry, writeBuffer);
    ...
    cacheEntry = readBuffer.load(fileId, 0, false, writeBuffer, 4, true, null);
    readBuffer.release(cacheEntry, writeBuffer);
  ...
  public void testPrefetchPagesInA1inAmQueue() throws Exception {
    ...
    for (int i = 0; i < 7; i++) {
      OCacheEntry cacheEntry = readBuffer.load(fileId, i, false, writeBuffer, 1, true, null);
      readBuffer.release(cacheEntry, writeBuffer);
    }
    ...
    OCacheEntry cacheEntry = readBuffer.load(fileId, 1, false, writeBuffer, 1, true, null);
    readBuffer.release(cacheEntry, writeBuffer);
  ...
  public void testPrefetchPagesInPinnedPages() throws Exception {
    ...
    cacheEntry = readBuffer.load(fileId, 4, false, writeBuffer, 1, true, null);
  ...
  public void testLoadAndLockForReadShouldHitCache() throws Exception {
    ...
    final OByteBufferPool bufferPool = OByteBufferPool.instance();
    final OCacheEntry entry = generateEntry(fileId, 0, cacheEntry.getCachePointer().getBufferDuplicate(), bufferPool, false,
        new OLogSequenceNumber(0, 0));
  ...
  public void testIfAllPagesAreUsedExceptionShouldBeThrown() throws Exception {
    ...
    try {
      for (int i = 0; i < 5; i++) {
        ...
        if (i - 4 >= 0) {
          readBuffer.load(fileId, i - 4, false, writeBuffer, 0, true, null);
