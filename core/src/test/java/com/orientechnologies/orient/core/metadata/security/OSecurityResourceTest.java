...
public class OSecurityResourceTest {
  ...
  public void testParse(){
    ...
    Assert.assertEquals("Person", ((OSecurityResourceClass)OSecurityResource.parseResource("database.class.Person")).getClassName());
    Assert.assertTrue(OSecurityResource.parseResource("database.class.Person.name") instanceof OSecurityResourceProperty);
    Assert.assertEquals("Person", ((OSecurityResourceProperty)OSecurityResource.parseResource("database.class.Person.name")).getClassName());
    Assert.assertEquals("name", ((OSecurityResourceProperty)OSecurityResource.parseResource("database.class.Person.name")).getPropertyName());
    ...
    Assert.assertEquals("person", ((OSecurityResourceCluster)OSecurityResource.parseResource("database.cluster.person")).getClusterName());
    Assert.assertTrue(OSecurityResource.parseResource("database.function.foo") instanceof OSecurityResourceFunction);
    Assert.assertEquals(OSecurityResourceDatabaseOp.BYPASS_RESTRICTED, OSecurityResource.parseResource("database.bypassRestricted"));
