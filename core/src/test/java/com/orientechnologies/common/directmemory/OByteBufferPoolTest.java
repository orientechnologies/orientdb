...
public class OByteBufferPoolTest {
  ...
  public void testByteBufferAllocationZeroPool() {
    ...
    final ByteBuffer bufferOne = byteBufferPool.acquireDirect(false, 0);
    ...
    final ByteBuffer bufferTwo = byteBufferPool.acquireDirect(true, 0);
  ...
  public void testByteBufferAllocationTwoPagesPool() {
    ...
    ByteBuffer bufferOne = byteBufferPool.acquireDirect(false, 0);

    Assert.assertEquals(42, bufferOne.capacity());
    Assert.assertEquals(0, byteBufferPool.getPoolSize());
    Assert.assertEquals(42, allocator.getMemoryConsumption());

    ByteBuffer bufferTwo = byteBufferPool.acquireDirect(true, 0);
    Assert.assertEquals(42, bufferTwo.capacity());
    Assert.assertEquals(0, byteBufferPool.getPoolSize());
    Assert.assertEquals(84, allocator.getMemoryConsumption());
    ...
    ByteBuffer bufferThree = byteBufferPool.acquireDirect(false, 0);
    ...
    bufferOne = byteBufferPool.acquireDirect(true, 0);
    ...
    bufferTwo = byteBufferPool.acquireDirect(true, 0);
    ...
    bufferThree = byteBufferPool.acquireDirect(false, 0);
    ...
    bufferThree = byteBufferPool.acquireDirect(true, 0);
      ...
      try {
        while (!stop.get()) {
          if (allocatedBuffers.size() < 500) {
            ByteBuffer buffer = pool.acquireDirect(false, 0);
