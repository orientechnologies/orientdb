...
public class OEmbeddedRidBag implements ORidBagDelegate {
  ...
  private OSimpleMultiValueChangeListener<OIdentifiable, OIdentifiable> changeListener = new OSimpleMultiValueChangeListener<>(this);
  ...
  private void addEvent(OIdentifiable key, OIdentifiable identifiable) {
    ...
    if (changeListener.isEnabled()) {
      changeListener.add(key, identifiable);
    } else {
      setDirty();
    }
  }
  ...
  private void updateEvent(OIdentifiable key, OIdentifiable oldValue, OIdentifiable newValue) {
    ...
    if (changeListener.isEnabled()) {
      changeListener.updated(key, oldValue, newValue);
    } else {
      setDirty();
    }
  }
