...
public class FindReferencesStep extends AbstractExecutionStep {
  ...
  public FindReferencesStep(List<OIdentifier> classes, List<OCluster> clusters, OCommandContext ctx) {
    super(ctx);
  ...
  private void initClusterIterators(OCommandContext ctx) {
    ...
    List<ORecordIteratorCluster> iterators = targetClusterNames.stream().map(
        clusterName -> new ORecordIteratorCluster((ODatabaseDocumentInternal) db, (ODatabaseDocumentInternal) db,
            db.getClusterIdByName(clusterName))).collect(Collectors.toList());
