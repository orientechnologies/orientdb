...
public abstract class OTreeMapEntryPersistent<K, V> extends OTreeMapEntry<K, V> implements OSerializableStream {
  ...
  protected OTreeMapEntry<K, V>      left;
  protected OTreeMapEntry<K, V>      right;
  ...
  public void clear() {
    ...
    if (parent != null)
      if (((OTreeMapEntryPersistent<K, V>) parent).left == this)
        ((OTreeMapEntryPersistent<K, V>) parent).left = null;
      else
        ((OTreeMapEntryPersistent<K, V>) parent).right = null;
  }
