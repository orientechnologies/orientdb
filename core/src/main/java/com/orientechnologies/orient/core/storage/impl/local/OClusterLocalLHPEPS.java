...
public class OClusterLocalLHPEPS extends OSharedResourceAdaptive implements OCluster {
  ...
  private static final double                                   storageOverflow        = 0.75;
  ...
  private void init(final OStorage iStorage, final int iId, final String iClusterName, final int iDataSegmentId) throws IOException {
    ...
    if (fileSegment == null) {
      ...
      config.setOverflowFile(new OStorageClusterLocalLHPEOverflowConfiguration(config, OStorageVariableParser.DB_PATH_VARIABLE
          + '/' + config.name, config.fileType, config.fileMaxSize));
      ...
      overflowSegment = new OClusterLocalLHPEOverflow(storage, config.getOverflowFile(), this);
  ...
  private void setNameInternal(final String iNewName) {
    ...
    overflowSegment.rename(name, iNewName);
  ...
  private void storeBuckets() throws IOException {
    for (OClusterLocalLHPEBucket mainBucket : mainBucketsToStore) {
      ...
      file.write(p, mainBucket.getBuffer());
    }
  ...
  private void allocateSpace(int space) throws IOException {
    fileSegment.allocateSpace(space);
  }
  ...
  private void splitBucketIfNeeded() throws IOException {
    ...
    if (currentCapacity > storageOverflow)
      splitBuckets();
  }
  ...
  private void splitBuckets() throws IOException {
    ...
    if (mainBucketsSize - 1 < positionToAdd) {
      allocateSpace((int) ((positionToAdd - mainBucketsSize + 1) * OClusterLocalLHPEBucket.BUCKET_SIZE_IN_BYTES));
