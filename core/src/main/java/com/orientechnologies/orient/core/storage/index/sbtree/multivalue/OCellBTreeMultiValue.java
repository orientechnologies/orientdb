...
public final class OCellBTreeMultiValue<K> extends ODurableComponent {
  ...
  public void create(final OBinarySerializer<K> keySerializer, final OType[] keyTypes, final int keySize,
    ...
    try {
      ...
      try {
        ...
        fileId = addFile(atomicOperation, getFullName(), true);

        nullBucketFileId = addFile(atomicOperation, getName() + nullFileExtension, true);

        final OCacheEntry entryPointCacheEntry = addPage(atomicOperation, fileId, true);
        try {
        ...
        final OCacheEntry rootCacheEntry = addPage(atomicOperation, fileId, true);
        try {
          ...
          if (freeListHeader >= 0) {
            ...
            try {
              ...
              if (nullBucket.isEmpty()) {
                final OCacheEntry cacheEntryPoint = loadPageForWrite(atomicOperation, nullBucketFileId, 0, false, true, true);
                try {
            ...
            try {
              ...
              if (getFilledUpTo(atomicOperation, nullBucketFileId, true) <= size + 1) {
                nullCacheEntry = addPage(atomicOperation, nullBucketFileId, true);
              } else {
                nullCacheEntry = loadPageForWrite(atomicOperation, nullBucketFileId, size + 1, false, false, true);
              }
              try {
  ...
  private void addToValueList(final OAtomicOperation atomicOperation, final ONullEntryPoint entryPointBucket, final int pageIndex)
    ...
    if (lastPage >= 0) {
      final OCacheEntry lastPageEntry = loadPageForWrite(atomicOperation, nullBucketFileId, lastPage, false, true, true);
      try {
  ...
  public void clear() throws IOException {
    ...
    try {
      acquireExclusiveLock();
      try {
        final OCacheEntry nullEntryPointCacheEntry = loadPageForWrite(atomicOperation, nullBucketFileId, 0, false, true, true);
        try {
  ...
  public void load(final String name, final int keySize, final OType[] keyTypes, final OBinarySerializer<K> keySerializer,
    ...
    try {
      ...
      fileId = openFile(atomicOperation, getFullName(), true);
    ...
    try {
      ...
      try {
        if (key != null) {
          ...
          while (leftSibling >= 0) {
            cacheEntry = loadPageForWrite(atomicOperation, fileId, leftSibling, false, true, true);
            try {
              final Bucket<K> bucket = new Bucket<>(cacheEntry, keySerializer, encryption);
              final int size = bucket.size();
          ...
          while (rightSibling >= 0) {
            cacheEntry = loadPageForWrite(atomicOperation, fileId, rightSibling, false, true, true);
            try {
              final Bucket<K> bucket = new Bucket<>(cacheEntry, keySerializer, encryption);
              final int size = bucket.size();
          ...
          {
            final OCacheEntry entryPointCacheEntry = loadPageForWrite(atomicOperation, nullBucketFileId, 0, false, true, true);
            try {
              final ONullEntryPoint entryPoint = new ONullEntryPoint(entryPointCacheEntry);
              entryPoint.setSize(0);
              entryPoint.setFirsPage(-1);
    ...
    try {
      ...
      try {
        if (key != null) {
          ...
          int prevPage = -1;
          while (currentPage >= 0) {
            final OCacheEntry nullCacheEntry = loadPageForWrite(atomicOperation, nullBucketFileId, currentPage, false, true, true);
            try {
              final ONullBucket nullBucket = new ONullBucket(nullCacheEntry, false);
  ...
  private UpdateBucketSearchResult splitNonRootBucket(final List<Long> path, final List<Integer> insertionIndexes,
    ...
    try {
      ...
      if (pageSize < getFilledUpTo(atomicOperation, fileId, true) - 1) {
        ...
        rightBucketEntry = addPage(atomicOperation, fileId, true);
  ...
  private UpdateBucketSearchResult splitRootBucket(final int keyIndex, final K keyToInsert, final OCacheEntry bucketEntry,
    ...
    try {
      ...
      if (pageSize < filledUpTo - 1) {
        ...
        leftBucketEntry = addPage(atomicOperation, fileId, true);
