...
public class OSQLAsynchQueryLocalExecutor implements OQueryExecutor {
	...
	public <R extends ORecordSchemaAware<?>> List<R> execute(final OQuery<R> iQuery, final int iLimit) {
		...
		// TODO: SUPPORTS MULTIPLE CLASS LIKE A SQL JOIN
		String firstClass = query.compiled.getClasses().size() > 0 ? query.compiled.getClasses().keySet().iterator().next() : null;
		...
		if (firstClass != null) {
			...
			String firstCluster = query.compiled.getClusters().size() > 0 ? query.compiled.getClusters().keySet().iterator().next()
					: null;
