...
import static com.orientechnologies.orient.core.record.impl.ODocumentHelper.ODbRelatedCall;
...
public class ODatabaseCompare extends ODatabaseImpExpAbstract {
	...
	private void compareIndexes() {
		...
		final Collection<? extends OIndex> indexesOne =
						makeDbCall(databaseDocumentTxOne, new ODbRelatedCall<Collection<? extends OIndex>>() {
		...
		final Iterator<? extends OIndex> iteratorOne =
						makeDbCall(databaseDocumentTxOne, new ODbRelatedCall<Iterator<? extends OIndex>>() {
			...
			if(compareEntriesForAutomaticIndexes || !indexOne.isAutomatic()) {
				...
				while (makeDbCall(databaseDocumentTxOne, new ODbRelatedCall<Boolean>() {
					...
					if(indexTwoValue == null) {
						ok = false;
						listener.onMessage("\n- ERR: Entree with key " + key + " is absent in index " + indexOne.getName()
										+ " for DB2.");
						++differences;
						continue;
					}
					...
					if(indexOneValue instanceof Set && indexTwoValue instanceof Set) {
					 ...
					 if(!ODocumentHelper.compareSets(databaseDocumentTxOne, indexOneValueSet, databaseDocumentTxTwo,
									 indexTwoValueSet)) {
							ok = false;
