...
public class OLocalHashTable<K, V> extends ODurableComponent implements OHashTable<K, V> {
  ...
  public void create(final OBinarySerializer<K> keySerializer, final OBinarySerializer<V> valueSerializer, final OType[] keyTypes,
    ...
    try {
      ...
      try {
        ...
        fileStateId = addFile(atomicOperation, getName() + metadataConfigurationFileExtension, true);

        directory.create(atomicOperation);

        final OCacheEntry hashStateEntry = addPage(atomicOperation, fileStateId, true);
        ...
        fileId = addFile(atomicOperation, fileName, true);
        ...
        if (nullKeyIsSupported) {
          nullBucketFileId = addFile(atomicOperation, getName() + nullBucketFileExtension, true);
        }
      } finally {
  ...
  public void setKeySerializer(final OBinarySerializer<K> keySerializer) throws IOException {
    ...
    try {
      ...
      try {
        ...
        final OCacheEntry hashStateEntry = loadPageForWrite(atomicOperation, fileStateId, hashStateEntryIndex, true, true, true);
        try {
    ...
    try {
      ...
      try {
        ...
        if (key != null) {
          ...
          OCacheEntry cacheEntry = loadPageForWrite(atomicOperation, nullBucketFileId, 0, false, true, true);
          if (cacheEntry == null) {
  ...
  public void load(final String name, final OType[] keyTypes, final boolean nullKeyIsSupported, final OEncryption encryption,
    ...
    try {
      ...
      fileStateId = openFile(atomicOperation, name + metadataConfigurationFileExtension, true);
      ...
      if (nullKeyIsSupported) {
        nullBucketFileId = openFile(atomicOperation, name + nullBucketFileExtension, true);
      }
    ...
    try {
      ...
      try {
        ...
        if (isFileExists(atomicOperation, name + nullBucketFileExtension)) {
          final long nullBucketId = openFile(atomicOperation, name + nullBucketFileExtension, true);
  ...
  private OHashTable.BucketSplitResult splitBucket(final OHashIndexBucket<K, V> bucket, final long pageIndex,
    ...
    final OCacheEntry newBucketCacheEntry = addPage(atomicOperation, fileId, true);
