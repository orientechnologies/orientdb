 ...
 * <p>
 * At the first step we perform fsync of the data to the log using single sequential write calling {@link #write(ByteBuffer[], long,
 * int)} method.
 * <p>
 * As the second step we write pages to the data files.
 * <p>
 * At third step we fsync data files in background process and then truncate log calling {@link #truncate()} method. Write to the
 ...
 * <p>
 * If during the write of pages we reach log threshold {@link #write(ByteBuffer[], long, int)} method will return true. If that
 * happens fsync of pages should be called to truncate page log.
 */
public interface DoubleWriteLog {
  boolean write(ByteBuffer[] buffers, long fileId, int pageIndex) throws IOException;
