...
public abstract class OAbstractCollectionConverter<T> implements OValuesConverter<T> {
  ...
  protected boolean convertSingleValue(final Object item, ResultCallback result, boolean updated) {
    ...
    if (item instanceof OIdentifiable) {
      final OValuesConverter<OIdentifiable> converter = (OValuesConverter<OIdentifiable>) OImportConvertersFactory.INSTANCE
          .getConverter(item, converterData);
      ...
      final OValuesConverter valuesConverter = OImportConvertersFactory.INSTANCE.getConverter(item.getClass(), converterData);
