/* Generated By:JJTree&JavaCC: Do not edit this line. OrientSqlConstants.java */
package com.orientechnologies.orient.core.sql.parser;


/**
 * Token literal values and constants.
 * Generated by org.javacc.parser.OtherFilesGen#start()
 */
public interface OrientSqlConstants {

  /** End of File. */
  int EOF = 0;
  /** RegularExpression Id. */
  int FORMAL_COMMENT = 8;
  /** RegularExpression Id. */
  int MULTI_LINE_COMMENT = 9;
  /** RegularExpression Id. */
  int SELECT = 11;
  /** RegularExpression Id. */
  int TRAVERSE = 12;
  /** RegularExpression Id. */
  int INSERT = 13;
  /** RegularExpression Id. */
  int CREATE = 14;
  /** RegularExpression Id. */
  int DELETE = 15;
  /** RegularExpression Id. */
  int VERTEX = 16;
  /** RegularExpression Id. */
  int EDGE = 17;
  /** RegularExpression Id. */
  int UPDATE = 18;
  /** RegularExpression Id. */
  int UPSERT = 19;
  /** RegularExpression Id. */
  int FROM = 20;
  /** RegularExpression Id. */
  int TO = 21;
  /** RegularExpression Id. */
  int WHERE = 22;
  /** RegularExpression Id. */
  int WHILE = 23;
  /** RegularExpression Id. */
  int INTO = 24;
  /** RegularExpression Id. */
  int VALUES = 25;
  /** RegularExpression Id. */
  int SET = 26;
  /** RegularExpression Id. */
  int ADD = 27;
  /** RegularExpression Id. */
  int PUT = 28;
  /** RegularExpression Id. */
  int MERGE = 29;
  /** RegularExpression Id. */
  int CONTENT = 30;
  /** RegularExpression Id. */
  int REMOVE = 31;
  /** RegularExpression Id. */
  int INCREMENT = 32;
  /** RegularExpression Id. */
  int AND = 33;
  /** RegularExpression Id. */
  int OR = 34;
  /** RegularExpression Id. */
  int NULL = 35;
  /** RegularExpression Id. */
  int DEFINED = 36;
  /** RegularExpression Id. */
  int ORDER = 37;
  /** RegularExpression Id. */
  int GROUP = 38;
  /** RegularExpression Id. */
  int BY = 39;
  /** RegularExpression Id. */
  int LIMIT = 40;
  /** RegularExpression Id. */
  int SKIP2 = 41;
  /** RegularExpression Id. */
  int BATCH = 42;
  /** RegularExpression Id. */
  int OFFSET = 43;
  /** RegularExpression Id. */
  int TIMEOUT = 44;
  /** RegularExpression Id. */
  int ASC = 45;
  /** RegularExpression Id. */
  int AS = 46;
  /** RegularExpression Id. */
  int DESC = 47;
  /** RegularExpression Id. */
  int FETCHPLAN = 48;
  /** RegularExpression Id. */
  int RETURN = 49;
  /** RegularExpression Id. */
  int BEFORE = 50;
  /** RegularExpression Id. */
  int AFTER = 51;
  /** RegularExpression Id. */
  int LOCK = 52;
  /** RegularExpression Id. */
  int RECORD = 53;
  /** RegularExpression Id. */
  int WAIT = 54;
  /** RegularExpression Id. */
  int RETRY = 55;
  /** RegularExpression Id. */
  int LET = 56;
  /** RegularExpression Id. */
  int NOCACHE = 57;
  /** RegularExpression Id. */
  int UNSAFE = 58;
  /** RegularExpression Id. */
  int PARALLEL = 59;
  /** RegularExpression Id. */
  int STRATEGY = 60;
  /** RegularExpression Id. */
  int DEPTH_FIRST = 61;
  /** RegularExpression Id. */
  int BREADTH_FIRST = 62;
  /** RegularExpression Id. */
  int LUCENE = 63;
  /** RegularExpression Id. */
  int NEAR = 64;
  /** RegularExpression Id. */
  int WITHIN = 65;
  /** RegularExpression Id. */
  int UNWIND = 66;
  /** RegularExpression Id. */
  int MAXDEPTH = 67;
  /** RegularExpression Id. */
  int EXCEPTION = 68;
  /** RegularExpression Id. */
  int THIS = 69;
  /** RegularExpression Id. */
  int RECORD_ATTRIBUTE = 70;
  /** RegularExpression Id. */
  int RID_ATTR = 71;
  /** RegularExpression Id. */
  int CLASS_ATTR = 72;
  /** RegularExpression Id. */
  int VERSION_ATTR = 73;
  /** RegularExpression Id. */
  int SIZE_ATTR = 74;
  /** RegularExpression Id. */
  int TYPE_ATTR = 75;
  /** RegularExpression Id. */
  int RAW_ATTR = 76;
  /** RegularExpression Id. */
  int RID_ID_ATTR = 77;
  /** RegularExpression Id. */
  int RID_POS_ATTR = 78;
  /** RegularExpression Id. */
  int FIELDS_ATTR = 79;
  /** RegularExpression Id. */
  int INTEGER_LITERAL = 80;
  /** RegularExpression Id. */
  int DECIMAL_LITERAL = 81;
  /** RegularExpression Id. */
  int HEX_LITERAL = 82;
  /** RegularExpression Id. */
  int OCTAL_LITERAL = 83;
  /** RegularExpression Id. */
  int FLOATING_POINT_LITERAL = 84;
  /** RegularExpression Id. */
  int DECIMAL_FLOATING_POINT_LITERAL = 85;
  /** RegularExpression Id. */
  int DECIMAL_EXPONENT = 86;
  /** RegularExpression Id. */
  int HEXADECIMAL_FLOATING_POINT_LITERAL = 87;
  /** RegularExpression Id. */
  int HEXADECIMAL_EXPONENT = 88;
  /** RegularExpression Id. */
  int CHARACTER_LITERAL = 89;
  /** RegularExpression Id. */
  int STRING_LITERAL = 90;
  /** RegularExpression Id. */
  int INTEGER_RANGE = 91;
  /** RegularExpression Id. */
  int TRUE = 92;
  /** RegularExpression Id. */
  int FALSE = 93;
  /** RegularExpression Id. */
  int LPAREN = 94;
  /** RegularExpression Id. */
  int RPAREN = 95;
  /** RegularExpression Id. */
  int LBRACE = 96;
  /** RegularExpression Id. */
  int RBRACE = 97;
  /** RegularExpression Id. */
  int LBRACKET = 98;
  /** RegularExpression Id. */
  int RBRACKET = 99;
  /** RegularExpression Id. */
  int SEMICOLON = 100;
  /** RegularExpression Id. */
  int COMMA = 101;
  /** RegularExpression Id. */
  int DOT = 102;
  /** RegularExpression Id. */
  int AT = 103;
  /** RegularExpression Id. */
  int DOLLAR = 104;
  /** RegularExpression Id. */
  int BACKTICK = 105;
  /** RegularExpression Id. */
  int EQ = 106;
  /** RegularExpression Id. */
  int EQEQ = 107;
  /** RegularExpression Id. */
  int LT = 108;
  /** RegularExpression Id. */
  int GT = 109;
  /** RegularExpression Id. */
  int BANG = 110;
  /** RegularExpression Id. */
  int TILDE = 111;
  /** RegularExpression Id. */
  int HOOK = 112;
  /** RegularExpression Id. */
  int COLON = 113;
  /** RegularExpression Id. */
  int LE = 114;
  /** RegularExpression Id. */
  int GE = 115;
  /** RegularExpression Id. */
  int NE = 116;
  /** RegularExpression Id. */
  int NEQ = 117;
  /** RegularExpression Id. */
  int SC_OR = 118;
  /** RegularExpression Id. */
  int SC_AND = 119;
  /** RegularExpression Id. */
  int INCR = 120;
  /** RegularExpression Id. */
  int DECR = 121;
  /** RegularExpression Id. */
  int PLUS = 122;
  /** RegularExpression Id. */
  int MINUS = 123;
  /** RegularExpression Id. */
  int STAR = 124;
  /** RegularExpression Id. */
  int SLASH = 125;
  /** RegularExpression Id. */
  int BIT_AND = 126;
  /** RegularExpression Id. */
  int BIT_OR = 127;
  /** RegularExpression Id. */
  int XOR = 128;
  /** RegularExpression Id. */
  int REM = 129;
  /** RegularExpression Id. */
  int LSHIFT = 130;
  /** RegularExpression Id. */
  int PLUSASSIGN = 131;
  /** RegularExpression Id. */
  int MINUSASSIGN = 132;
  /** RegularExpression Id. */
  int STARASSIGN = 133;
  /** RegularExpression Id. */
  int SLASHASSIGN = 134;
  /** RegularExpression Id. */
  int ANDASSIGN = 135;
  /** RegularExpression Id. */
  int ORASSIGN = 136;
  /** RegularExpression Id. */
  int XORASSIGN = 137;
  /** RegularExpression Id. */
  int REMASSIGN = 138;
  /** RegularExpression Id. */
  int LSHIFTASSIGN = 139;
  /** RegularExpression Id. */
  int RSIGNEDSHIFTASSIGN = 140;
  /** RegularExpression Id. */
  int RUNSIGNEDSHIFTASSIGN = 141;
  /** RegularExpression Id. */
  int ELLIPSIS = 142;
  /** RegularExpression Id. */
  int RANGE = 143;
  /** RegularExpression Id. */
  int NOT = 144;
  /** RegularExpression Id. */
  int IN = 145;
  /** RegularExpression Id. */
  int LIKE = 146;
  /** RegularExpression Id. */
  int IS = 147;
  /** RegularExpression Id. */
  int BETWEEN = 148;
  /** RegularExpression Id. */
  int CONTAINS = 149;
  /** RegularExpression Id. */
  int CONTAINSALL = 150;
  /** RegularExpression Id. */
  int CONTAINSKEY = 151;
  /** RegularExpression Id. */
  int CONTAINSVALUE = 152;
  /** RegularExpression Id. */
  int CONTAINSTEXT = 153;
  /** RegularExpression Id. */
  int MATCHES = 154;
  /** RegularExpression Id. */
  int KEY = 155;
  /** RegularExpression Id. */
  int INSTANCEOF = 156;
  /** RegularExpression Id. */
  int CLUSTER = 157;
  /** RegularExpression Id. */
  int IDENTIFIER = 158;
  /** RegularExpression Id. */
  int QUOTED_IDENTIFIER = 159;
  /** RegularExpression Id. */
  int INDEX_IDENTIFIER = 160;
  /** RegularExpression Id. */
  int INDEXVALUES_IDENTIFIER = 161;
  /** RegularExpression Id. */
  int INDEXVALUESASC_IDENTIFIER = 162;
  /** RegularExpression Id. */
  int INDEXVALUESDESC_IDENTIFIER = 163;
  /** RegularExpression Id. */
  int CLUSTER_IDENTIFIER = 164;
  /** RegularExpression Id. */
  int CLUSTER_NUMBER_IDENTIFIER = 165;
  /** RegularExpression Id. */
  int METADATA_IDENTIFIER = 166;
  /** RegularExpression Id. */
  int NAMED_PARAMETER = 167;
  /** RegularExpression Id. */
  int LETTER = 168;
  /** RegularExpression Id. */
  int PART_LETTER = 169;

  /** Lexical state. */
  int DEFAULT = 0;
  /** Lexical state. */
  int IN_FORMAL_COMMENT = 1;
  /** Lexical state. */
  int IN_MULTI_LINE_COMMENT = 2;

  /** Literal token values. */
  String[] tokenImage = {
    "<EOF>",
    "\" \"",
    "\"\\t\"",
    "\"\\n\"",
    "\"\\r\"",
    "\"\\f\"",
    "<token of kind 6>",
    "\"/*\"",
    "\"*/\"",
    "\"*/\"",
    "<token of kind 10>",
    "<SELECT>",
    "<TRAVERSE>",
    "<INSERT>",
    "<CREATE>",
    "<DELETE>",
    "<VERTEX>",
    "<EDGE>",
    "<UPDATE>",
    "<UPSERT>",
    "<FROM>",
    "<TO>",
    "<WHERE>",
    "<WHILE>",
    "<INTO>",
    "<VALUES>",
    "<SET>",
    "<ADD>",
    "<PUT>",
    "<MERGE>",
    "<CONTENT>",
    "<REMOVE>",
    "<INCREMENT>",
    "<AND>",
    "<OR>",
    "<NULL>",
    "<DEFINED>",
    "<ORDER>",
    "<GROUP>",
    "<BY>",
    "<LIMIT>",
    "<SKIP2>",
    "<BATCH>",
    "<OFFSET>",
    "<TIMEOUT>",
    "<ASC>",
    "<AS>",
    "<DESC>",
    "<FETCHPLAN>",
    "<RETURN>",
    "<BEFORE>",
    "<AFTER>",
    "<LOCK>",
    "<RECORD>",
    "<WAIT>",
    "<RETRY>",
    "<LET>",
    "<NOCACHE>",
    "<UNSAFE>",
    "<PARALLEL>",
    "<STRATEGY>",
    "<DEPTH_FIRST>",
    "<BREADTH_FIRST>",
    "<LUCENE>",
    "<NEAR>",
    "<WITHIN>",
    "<UNWIND>",
    "<MAXDEPTH>",
    "<EXCEPTION>",
    "<THIS>",
    "<RECORD_ATTRIBUTE>",
    "<RID_ATTR>",
    "<CLASS_ATTR>",
    "<VERSION_ATTR>",
    "<SIZE_ATTR>",
    "<TYPE_ATTR>",
    "<RAW_ATTR>",
    "<RID_ID_ATTR>",
    "<RID_POS_ATTR>",
    "<FIELDS_ATTR>",
    "<INTEGER_LITERAL>",
    "<DECIMAL_LITERAL>",
    "<HEX_LITERAL>",
    "<OCTAL_LITERAL>",
    "<FLOATING_POINT_LITERAL>",
    "<DECIMAL_FLOATING_POINT_LITERAL>",
    "<DECIMAL_EXPONENT>",
    "<HEXADECIMAL_FLOATING_POINT_LITERAL>",
    "<HEXADECIMAL_EXPONENT>",
    "<CHARACTER_LITERAL>",
    "<STRING_LITERAL>",
    "<INTEGER_RANGE>",
    "\"true\"",
    "\"false\"",
    "\"(\"",
    "\")\"",
    "\"{\"",
    "\"}\"",
    "\"[\"",
    "\"]\"",
    "\";\"",
    "\",\"",
    "\".\"",
    "\"@\"",
    "\"$\"",
    "\"`\"",
    "\"=\"",
    "\"==\"",
    "\"<\"",
    "\">\"",
    "\"!\"",
    "\"~\"",
    "\"?\"",
    "\":\"",
    "\"<=\"",
    "\">=\"",
    "\"!=\"",
    "\"<>\"",
    "\"||\"",
    "\"&&\"",
    "\"++\"",
    "\"--\"",
    "\"+\"",
    "\"-\"",
    "\"*\"",
    "\"/\"",
    "\"&\"",
    "\"|\"",
    "\"^\"",
    "\"%\"",
    "\"<<\"",
    "\"+=\"",
    "\"-=\"",
    "\"*=\"",
    "\"/=\"",
    "\"&=\"",
    "\"|=\"",
    "\"^=\"",
    "\"%=\"",
    "\"<<=\"",
    "\">>=\"",
    "\">>>=\"",
    "\"...\"",
    "\"..\"",
    "<NOT>",
    "<IN>",
    "<LIKE>",
    "<IS>",
    "<BETWEEN>",
    "<CONTAINS>",
    "<CONTAINSALL>",
    "<CONTAINSKEY>",
    "<CONTAINSVALUE>",
    "<CONTAINSTEXT>",
    "<MATCHES>",
    "<KEY>",
    "<INSTANCEOF>",
    "<CLUSTER>",
    "<IDENTIFIER>",
    "<QUOTED_IDENTIFIER>",
    "<INDEX_IDENTIFIER>",
    "<INDEXVALUES_IDENTIFIER>",
    "<INDEXVALUESASC_IDENTIFIER>",
    "<INDEXVALUESDESC_IDENTIFIER>",
    "<CLUSTER_IDENTIFIER>",
    "<CLUSTER_NUMBER_IDENTIFIER>",
    "<METADATA_IDENTIFIER>",
    "<NAMED_PARAMETER>",
    "<LETTER>",
    "<PART_LETTER>",
    "\"#\"",
  };

}
