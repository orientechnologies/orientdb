...
public final class OSBTree<K, V> extends ODurableComponent {
  ...
  public void putRollback(byte[] rawKey, byte[] rawValue, OAtomicOperation atomicOperation) {
    ...
    acquireExclusiveLock();
    try {
      OComponentOperation componentOperation;
      ...
      if (key != null) {
        ...
        assert rawValue.length <= MAX_EMBEDDED_VALUE_SIZE;
        ...
        if (bucketSearchResult.itemIndex >= 0) {
          ...
          if (oldRawValue.length == rawValue.length) {
            keyBucket.updateValue(bucketSearchResult.itemIndex, rawValue, oldRawValue);
            releasePageFromWrite(keyBucket, atomicOperation);

            logComponentOperation(atomicOperation,
                new OSBTreePutOperation(atomicOperation.getOperationUnitId(), getName(), rawKey, rawValue, oldRawValue));
            endAtomicOperation(false, null);
            return;
          } else {
        ...
        componentOperation = new OSBTreePutOperation(atomicOperation.getOperationUnitId(), getName(), rawKey, rawValue,
            oldRawValue);
        ...
        try {
          if (!needNew) {
            ...
            nullBucket = new ONullBucket<>(cacheEntry, valueSerializer, true);
        ...
        if (updatedValue.isChange()) {
          ...
          if (bucketSearchResult.itemIndex >= 0) {
            ...
            if (oldRawValue.length == serializeValue.length) {
              ...
              logComponentOperation(atomicOperation,
                  new OSBTreePutOperation(atomicOperation.getOperationUnitId(), getName(), rawKey, serializeValue, oldRawValue));
              endAtomicOperation(false, null);
              return true;
            } else {
              keyBucket.remove(bucketSearchResult.itemIndex, rawKey, oldRawValue);
          ...
          componentOperation = new OSBTreePutOperation(atomicOperation.getOperationUnitId(), getName(), rawKey, serializeValue,
              oldRawValue);
          ...
          componentOperation = new OSBTreeRemoveOperation(atomicOperation.getOperationUnitId(), getName(), rawKey, oldRawValue);
  ...
  public void deleteRollback(@SuppressWarnings("unused") OAtomicOperation atomicOperation) {
    ...
    try {
      ...
      throw OException.wrapException(new OSBTreeException("Error during deletion of sbtree with name " + getName(), this), e);
    } finally {
      releaseExclusiveLock();
    }
  }
  ...
  public void removeRollback(byte[] rawKey, OAtomicOperation atomicOperation) {
    ...
    if (rawKey != null) {
      if (encryption != null) {
        key = keySerializer.deserializeNativeObject(
            encryption.decrypt(rawKey, OIntegerSerializer.INT_SIZE, rawKey.length - OIntegerSerializer.INT_SIZE), 0);
      } else {
        key = keySerializer.deserializeNativeObject(rawKey, 0);
      }
    } else {
    ...
    try {
      ...
      if (key != null) {
        ...
        BucketSearchResult bucketSearchResult = findBucket(key);
        assert bucketSearchResult.itemIndex >= 0;
        removedValue = removeKey(bucketSearchResult, atomicOperation);
      } else {
  ...
  private BucketSearchResult splitNonRootBucket(List<Long> path, int keyIndex, K keyToInsert, long pageIndex,
    ...
    try {
      newRightBucket = new OSBTreeBucket<>(rightBucketEntry, splitLeaf, keySerializer, keyTypes, valueSerializer, encryption);
  ...
  private BucketSearchResult splitRootBucket(List<Long> path, int keyIndex, K keyToInsert, OCacheEntry bucketEntry,
    ...
    OCacheEntry rightBucketEntry = addPage(fileId);
    try {
      newLeftBucket = new OSBTreeBucket<>(leftBucketEntry, splitLeaf, keySerializer, keyTypes, valueSerializer, encryption);
