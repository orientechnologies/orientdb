 ...
 * @since 8/7/13
 */
public class OSBTreeV2<K, V> extends ODurableComponent implements com.orientechnologies.orient.core.storage.index.sbtree.local.OSBTree<K, V> {
  ...
  public OSBTreeV2(final String name, final String dataFileExtension, final String nullFileExtension, final OAbstractPaginatedStorage storage) {
  ...
  private long allocateValuePageFromFreeList(final OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      rootBucket = new OSBTreeBucket<>(rootCacheEntry, keySerializer, keyTypes, valueSerializer);
  ...
  private BucketSearchResult splitNonRootBucket(final List<Long> path, final int keyIndex, final K keyToInsert, final long pageIndex, final OSBTreeBucket<K, V> bucketToSplit,
    ...
    try {
      final OSBTreeBucket<K, V> newRightBucket = new OSBTreeBucket<>(rightBucketEntry, splitLeaf, keySerializer, keyTypes, valueSerializer);
      ...
      if (splitLeaf) {
        ...
        if (rightSiblingPageIndex >= 0) {
          ...
          final OSBTreeBucket<K, V> rightSiblingBucket = new OSBTreeBucket<>(rightSiblingBucketEntry, keySerializer, keyTypes, valueSerializer);
          try {
  ...
  private BucketSearchResult splitRootBucket(final List<Long> path, final int keyIndex, final K keyToInsert, final OCacheEntry bucketEntry, OSBTreeBucket<K, V> bucketToSplit,
    ...
    final OCacheEntry rightBucketEntry = addPage(atomicOperation, fileId);
    try {
      final OSBTreeBucket<K, V> newLeftBucket = new OSBTreeBucket<>(leftBucketEntry, splitLeaf, keySerializer, keyTypes, valueSerializer);
