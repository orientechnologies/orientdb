                  ...
                  if (partUpperCase.startsWith(op.keyword + "(")) {
                     ...
                     if (op.minArguments > 0) {
                        ...
                        if (arguments.size() < op.minArguments || arguments.size() > op.maxArguments)
                           throw new OQueryParsingException(iQueryToParse.text, "Syntax error: field operator '" + op.keyword
                                 + "' needs "
               ...
               if (operator == OSQLFilterFieldOperator.SIZE.id)
                  ...
                  if (ioResult != null) {
                     if (ioResult instanceof Long)
                        ...
                        ioResult = ODatabaseRecordThreadLocal.INSTANCE.get().getStorage().getConfiguration()
                              .getDateFormatInstance().parse(ioResult.toString());
                  }
                  ...
                  if (ioResult != null) {
                     if (ioResult instanceof Long)
                        ...
                        ioResult = ODatabaseRecordThreadLocal.INSTANCE.get().getStorage().getConfiguration()
                              .getDateTimeFormatInstance().parse(ioResult.toString());
                  }
