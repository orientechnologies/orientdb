...
public class OGraphRepair {
  ...
  public void repair(final ODatabaseSession graph, final OCommandOutputListener outputListener,
  ...
  public void check(final ODatabaseSession graph, final OCommandOutputListener outputListener,
      final Map<String, List<String>> options) {
  ...
  protected void repairEdges(final ODatabaseSession graph, final ORepairStats stats, final OCommandOutputListener outputListener,
      final Map<String, List<String>> options, final boolean checkOnly) {
...
//    final OrientConfigurableGraph.Settings settings = graph.settings;
    ...
    if (edgeClass != null) {
      ...
      for (ODocument edge : db.browseClass(edgeClass.getName())) {
        ...
        if (outputListener != null && stats.scannedEdges % 100000 == 0) {
          ...
          message(outputListener,
              "+ edges: scanned " + stats.scannedEdges + ", removed " + stats.removedEdges + " (estimated remaining time "
                  + remaining + " secs)\n");
        }
        ...
        if (out == null)
          ...
          if (outVertex == null)
            ...
            final String outFieldName = OVertexDocument
                .getConnectionFieldName(ODirection.OUT, edge.getClassName(), useVertexFieldsForEdgeLabels);
        ...
        if (in == null)
          ...
          if (inVertex == null)
            ...
            final String inFieldName = OVertexDocument
                .getConnectionFieldName(ODirection.IN, edge.getClassName(), useVertexFieldsForEdgeLabels);
        ...
        if (outVertexMissing || inVertexMissing) {
          try {
            if (!checkOnly) {
              ...
              message(outputListener, "+ found corrupted edge " + edge + " because " + removalReason + "\n");
  ...
  protected void repairVertices(final ODatabaseSession graph, final ORepairStats stats, final OCommandOutputListener outputListener,
      final Map<String, List<String>> options, final boolean checkOnly) {
    final ODatabaseDocument db = graph;
    final OMetadata metadata = db.getMetadata();
    final OSchema schema = metadata.getSchema();
    ...
    if (vertexClass != null) {
      ...
      for (ODocument vertex : db.browseClass(vertexClass.getName())) {
        ...
        for (String fieldName : vertex.fieldNames()) {
          final OPair<ODirection, String> connection = getConnection(db, ODirection.BOTH, fieldName, null);
          if (connection == null)
            // SKIP THIS FIELD
            continue;
  ...
  protected OPair<ODirection, String> getConnection(ODatabaseDocument graph, final ODirection iDirection, final String iFieldName,
      String... iClassNames) {
  ...
  private boolean isEdgeBroken(final OIdentifiable vertex, final String fieldName, final ODirection direction,
      ...
      if (record == null)
        ...
        if (immutableClass == null || (!immutableClass.isVertexType() && !immutableClass.isEdgeType()))
          ...
          if (immutableClass.isVertexType()) {
            ...
            final String inverseFieldName = getInverseConnectionFieldName(fieldName, useVertexFieldsForEdgeLabels);
