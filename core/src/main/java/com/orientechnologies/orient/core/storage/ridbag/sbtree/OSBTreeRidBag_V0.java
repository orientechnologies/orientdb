...
public class OSBTreeRidBag_V0 implements OSBTreeRidBag{
  ...
  private final class RIDBagIterator implements Iterator<OIdentifiable>, OResettable, OSizeable, OAutoConvertToRecord {
    ...
    public void remove() {
      ...
      if (OSBTreeRidBag_V0.this.owner != null)
        ORecordInternal.unTrack(OSBTreeRidBag_V0.this.owner, currentValue);
    ...
    @Override
    public int size() {
      return OSBTreeRidBag_V0.this.size();
    }
  ...
  public int serialize(BytesContainer bytes, UUID ownerUuid) {    
    ...
    if (context == null) {      
      ...
      //third int allocated
      bytes.offset = offset;
      bytes.offset = bytes.alloc(OIntegerSerializer.INT_SIZE);
      stream = bytes.bytes;
      offset = bytes.offset;
