 ...
 * @since 5/14/14
 */
public final class OHashTableDirectoryV2 extends ODurableComponent {
  ...
  OHashTableDirectoryV2(final String defaultExtension, final String name, final String lockName,
  ...
  private void init(final OAtomicOperation atomicOperation) throws IOException {
    ...
    ODirectoryFirstPageV2 firstPage = null;
    try {
      firstPage = new ODirectoryFirstPageV2(firstEntry);
  ...
  int addNewNode(final byte maxLeftChildDepth, final byte maxRightChildDepth, final byte nodeLocalDepth, final long[] newNode,
    ...
    try {
      ...
      if (nodeIndex < ODirectoryFirstPageV2.NODES_PER_PAGE) {
        ...
        final int pageIndex = nodeIndex / ODirectoryPageV2.NODES_PER_PAGE;
        final int localLevel = nodeIndex % ODirectoryPageV2.NODES_PER_PAGE;
        ...
        ODirectoryPageV2 page = null;
        try {
          page = new ODirectoryPageV2(cacheEntry);
  ...
  void deleteNode(final int nodeIndex, final OAtomicOperation atomicOperation) throws IOException {
    ...
    try {
      ...
      if (nodeIndex < ODirectoryFirstPageV2.NODES_PER_PAGE) {
        ...
        final int pageIndex = nodeIndex / ODirectoryPageV2.NODES_PER_PAGE;
        final int localNodeIndex = nodeIndex % ODirectoryPageV2.NODES_PER_PAGE;
  ...
  byte getMaxLeftChildDepth(final int nodeIndex) throws IOException {
    final ODirectoryPageV2 page = loadPage(nodeIndex, false);
    try {
  ...
  void setMaxLeftChildDepth(final int nodeIndex, final byte maxLeftChildDepth, final OAtomicOperation atomicOperation)
      throws IOException {
    final ODirectoryPageV2 page = loadPage(nodeIndex, true);
    try {
  ...
  private ODirectoryPageV2 loadPage(final int nodeIndex, final boolean exclusiveLock) throws IOException {
    if (nodeIndex < ODirectoryFirstPageV2.NODES_PER_PAGE) {
      ...
      return new ODirectoryFirstPageV2(cacheEntry);
    }
    ...
    return new ODirectoryPageV2(cacheEntry);
  }

  private void releasePage(final ODirectoryPageV2 page, final boolean exclusiveLock, final OAtomicOperation atomicOperation) {
  ...
  private static int getLocalNodeIndex(final int nodeIndex) {
    ...
    return (nodeIndex - ODirectoryFirstPageV2.NODES_PER_PAGE) % ODirectoryPageV2.NODES_PER_PAGE;
  }
}
