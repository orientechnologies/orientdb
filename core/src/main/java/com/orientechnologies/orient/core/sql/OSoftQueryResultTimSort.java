...
class OSoftQueryResultTimSort<T> {
  ...
  /**
   * Temp storage for merges. A workspace array may optionally be provided in constructor, and if so will be used as long as it is
   * big enough.
   */
  private SoftReference<T>[] tmp;
   ...
   */
  private OSoftQueryResultTimSort(SoftReference<T>[] a, Comparator<? super T> c, SoftReference<T>[] work, ReferenceQueue<T> queue,
      String query, int workBase, int workLen) {
    this.a = a;
    this.c = c;
    ...
    if (work == null || workLen < tlen || workBase + tlen > work.length) {
      ...
      SoftReference<T>[] newArray = (SoftReference<T>[]) java.lang.reflect.Array.newInstance(SoftReference.class, tlen);
      tmp = newArray;
      tmpBase = 0;
      tmpLen = tlen;
    } else {
  ...
  /**
   * Sorts the given range, using the given workspace array slice for temp storage when possible. This method is designed to be
   * invoked from public methods (in class Arrays) after performing any necessary array bounds checks and expanding parameters into
   * the required forms.
   *
   * @param a  the array to be sorted
  ...
  static <T> void sort(SoftReference<T>[] a, int hi, Comparator<? super T> c, ReferenceQueue<T> queue,
    ...
    if (nRemaining < MIN_MERGE) {
      int initRunLen = countRunAndMakeAscending(a, 0, hi, c, queue, query);
      binarySort(a, 0, hi, initRunLen, c, queue, query);
      return;
    }
     ...
     */
    OSoftQueryResultTimSort<T> ts = new OSoftQueryResultTimSort<T>(a, c, null, queue, query, 0, 0);
    int minRun = minRunLength(nRemaining);
    int lo = 0;
    do {
      // Identify next run
      int runLen = countRunAndMakeAscending(a, lo, hi, c, queue, query);

      // If run is short, extend to min(minRun, nRemaining)
      if (runLen < minRun) {
        int force = nRemaining <= minRun ? nRemaining : minRun;
        binarySort(a, lo, lo + force, lo + runLen, c, queue, query);
        runLen = force;
      }
  ...
  private static void throwCanExecuteException(String query) {
    if (query != null) {
  ...
  private static <T> void binarySort(SoftReference<T>[] a, int lo, int hi, int start, Comparator<? super T> c,
    ...
    if (start == lo)
      start++;
    for (; start < hi; start++) {
      SoftReference<T> pivotReference = a[start];
      T pivot = getItem(a[start], queue, query);
      ...
      while (left < right) {
        ...
        if (c.compare(pivot, getItem(a[mid], queue, query)) < 0)
          right = mid;
        else
          left = mid + 1;
      }
  ...
  private static <T> int countRunAndMakeAscending(SoftReference<T>[] a, int lo, int hi,
    ...
    if (c.compare(getItem(a[runHi++], queue, query), getItem(a[lo], queue, query)) < 0) { // Descending
      while (runHi < hi && c.compare(getItem(a[runHi], queue, query), getItem(a[runHi - 1], queue, query)) < 0)
        runHi++;
   ...
   * with {@link #binarySort}.
   * <p>
   * Roughly speaking, the computation is:
   * <p>
   * If n < MIN_MERGE, return n (it's too small to bother with fancy stuff). Else if n is an exact power of 2, return MIN_MERGE/2.
   ...
   * <p>
   * This method is called each time a new run is pushed onto the stack, so the invariants are guaranteed to hold for i < stackSize
  ...
  private void mergeAt(int i) {
    ...
    stackSize--;
         ...
         */
    int k = gallopRight(a[base2], a, base1, len1, 0, c, queue, query);
    assert k >= 0;
    base1 += k;
    len1 -= k;
    if (len1 == 0)
      return;
         ...
         */
    len2 = gallopLeft(a[base1 + len1 - 1], a, base2, len2, len2 - 1, c, queue, query);
    assert len2 >= 0;
    if (len2 == 0)
      return;
  ...
  private static <T> int gallopLeft(SoftReference<T> key, SoftReference<T>[] a, int base, int len, int hint,
    ...
    int lastOfs = 0;
    int ofs = 1;
    if (c.compare(getItem(key, queue, query), getItem(a[base + hint], queue, query)) > 0) {
      // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
      int maxOfs = len - hint;
      while (ofs < maxOfs && c.compare(getItem(key, queue, query), getItem(a[base + hint + ofs], queue, query)) > 0) {
        lastOfs = ofs;
        ofs = (ofs << 1) + 1;
    ...
    while (lastOfs < ofs) {
      ...
      if (c.compare(getItem(key, queue, query), getItem(a[base + m], queue, query)) > 0)
   ...
   * element of the second run (a[base1] > a[base2]), and the last element of the first run (a[base1 + len1-1]) must be greater than
  ...
  private void mergeLo(int base1, int len1, int base2, int len2) {
    ...
    SoftReference<T>[] a = this.a; // For performance
    SoftReference<T>[] tmp = ensureCapacity(len1);
    ...
    while (true) {
      ...
      do {
        ...
        if (c.compare(getItem(a[cursor2], queue, query), getItem(tmp[cursor1], queue, query)) < 0) {
          a[dest++] = a[cursor2++];
          count2++;
          count1 = 0;
          if (--len2 == 0)
      ...
      do {
        assert len1 > 1 && len2 > 0;
        count1 = gallopRight(a[cursor2], tmp, cursor1, len1, 0, c, queue, query);
        if (count1 != 0) {
        ...
        count2 = gallopLeft(tmp[cursor1], a, cursor2, len2, 0, c, queue, query);
        if (count2 != 0) {
  ...
  /**
   * Like mergeLo, except that this method should be called only if len1 >= len2; mergeLo should be called if len1 <= len2.  (Either
  ...
  private void mergeHi(int base1, int len1, int base2, int len2) {
    ...
    SoftReference<T>[] a = this.a; // For performance
    SoftReference<T>[] tmp = ensureCapacity(len2);
    ...
    while (true) {
      ...
      do {
        ...
        if (c.compare(getItem(tmp[cursor2], queue, query), getItem(a[cursor1], queue, query)) < 0) {
          a[dest--] = a[cursor1--];
          count1++;
          count2 = 0;
          if (--len1 == 0)
      ...
      do {
        assert len1 > 0 && len2 > 1;
        count1 = len1 - gallopRight(tmp[cursor2], a, base1, len1, len1 - 1, c, queue, query);
        if (count1 != 0) {
          dest -= count1;
          cursor1 -= count1;
          len1 -= count1;
        ...
        count2 = len2 - gallopLeft(a[cursor1], tmp, tmpBase, len2, len2 - 1, c, queue, query);
        if (count2 != 0) {
          dest -= count2;
          cursor2 -= count2;
          len2 -= count2;
  ...
  /**
   * Ensures that the external array tmp has at least the specified number of elements, increasing its size if necessary.  The size
   ...
   */
  private SoftReference<T>[] ensureCapacity(int minCapacity) {
    if (tmpLen < minCapacity) {
      ...
      @SuppressWarnings({ "unchecked", "UnnecessaryLocalVariable" })
      SoftReference<T>[] newArray = (SoftReference<T>[]) java.lang.reflect.Array.newInstance(SoftReference.class, newSize);
      tmp = newArray;
      tmpLen = newSize;
      tmpBase = 0;
    }
