...
class OSoftQueryResultTimSort<E> {
   ...
   * arrays, while offering performance comparable to a traditional mergesort when run on random arrays.  Like all proper
   * mergesorts, this sort is stable and runs O(n log n) time (worst case).  In the worst case, this sort requires temporary storage
   * space for n/2 object references; in the best case, it requires only a small constant amount of space.
   * <p>
  ...
  private void binarySort(SoftReference<E>[] a, int lo, int hi, int start, Comparator<? super E> c) {
    ...
    for (; start < hi; start++) {
      ...
      while (left < right) {
        ...
        if (c.compare(getItem(pivot), getItem(a[mid])) < 0)
          right = mid;
        else
          left = mid + 1;
      }
  ...
  private int countRunAndMakeAscending(SoftReference<E>[] a, int lo, int hi, Comparator<? super E> c) {
    ...
    if (c.compare(getItem(a[runHi++]), getItem(a[lo])) < 0) { // Descending
      while (runHi < hi && c.compare(getItem(a[runHi]), getItem(a[runHi - 1])) < 0)
        runHi++;
  ...
  private class SortState {
     ...
     */
    private SoftReference<E>[] tmp; // Actual runtime type will be Object[], regardless of T
    ...
    private SortState(SoftReference<E>[] a, Comparator<? super E> c, int len) {
      ...
      tmp = new SoftReference[tmpLength];
     ...
     * + n] is infinity. In other words, key belongs at index b + k; or in other words, the first k elements of a should precede
    ...
    private int gallopLeft(SoftReference<E> key, SoftReference<E>[] a, int base, int len, int hint, Comparator<? super E> c) {
      ...
      if (c.compare(getItem(key), getItem(a[base + hint])) > 0) {
        // Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
        int maxOfs = len - hint;
        while (ofs < maxOfs && c.compare(getItem(key), getItem(a[base + hint + ofs])) > 0) {
          lastOfs = ofs;
          ofs = (ofs << 1) + 1;
      ...
      while (lastOfs < ofs) {
        ...
        if (c.compare(getItem(key), getItem(a[base + m])) > 0)
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
      ...
      SoftReference<E>[] a = this.a; // For performance
      SoftReference<E>[] tmp = ensureCapacity(len1);
      ...
      Comparator<? super E> c = this.c;  // Use local variable for performance
      ...
      while (true) {
        ...
        do {
          ...
          if (c.compare(getItem(a[cursor2]), getItem(tmp[cursor1])) < 0) {
            a[dest++] = a[cursor2++];
            count2++;
            count1 = 0;
            if (--len2 == 0)
    ...
    private void mergeHi(int base1, int len1, int base2, int len2) {
      ...
      SoftReference<E>[] a = this.a; // For performance
      SoftReference<E>[] tmp = ensureCapacity(len2);
      ...
      while (true) {
        ...
        do {
          ...
          if (c.compare(getItem(tmp[cursor2]), getItem(a[cursor1])) < 0) {
            a[dest--] = a[cursor1--];
            count1++;
            count2 = 0;
            if (--len1 == 0)
