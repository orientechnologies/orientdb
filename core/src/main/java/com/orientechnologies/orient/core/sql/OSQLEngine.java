...
public class OSQLEngine {
	...
	public synchronized OQueryOperator[] getRecordOperators() {
		if (SORTED_OPERATORS != null) { return SORTED_OPERATORS; }
		...
		if (!operators.isEmpty()) { throw new OException("Unvalid sorting. " + OCollections.toString(pairs)); }
	...
	public OSQLFunction getFunction(String iFunctionName) {
		...
		while (ite.hasNext()) {
			...
			if (factory.getFunctionNames().contains(iFunctionName)) { return factory.createFunction(iFunctionName); }
		}
	...
	public OCommandExecutorSQLAbstract getCommand(final String candidate) {
		...
		if (found) {
			...
			while (ite.hasNext()) {
				...
				if (factory.getCommandNames().contains(commandName)) { return factory.createCommand(commandName); }
			}
		}
