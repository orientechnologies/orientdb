...
public class ODatabaseFullSyncSender {
  ...
  public ODatabaseFullSyncSender(OrientDBDistributed databases, ONodeIdentity requester, String database) {
  ...
  public void run() {
    new Thread(() -> {
      ...
      OutputStream outputStream = new BufferedOutputStream(new OutputStream() {
        @Override
        public void write(byte[] b, int off, int len) throws IOException {
          network.send(requester, new ODatabaseFullSyncChunk(database, uuid, b, len));
        }
      ...
      if (incremental) {
        ...
        try {
          int compression = session.getConfiguration()
