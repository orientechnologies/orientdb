...
public class OElectionContext {
  ...
  public synchronized Optional<OLogId> received(ONodeIdentity sender, String database, UUID electionId, Optional<OLogId> id) {
  ...
  public synchronized Optional<OElectionReply> receivedLastLog(ONodeIdentity sender, UUID electionId, String database,
      Optional<OLogId> id) {
    OElection election = elections.get(database);
    if (election != null) {
