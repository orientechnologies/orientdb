...
import com.orientechnologies.orient.distributed.impl.proxy.OProxyServer;
...
public class SplitBraiNetworkTestTempIT extends AbstractHARemoveNode {
  ...
  protected void startProxies() {
          ...
          case 1:
            if (b == 0) {
              ...
              if (port >= 2434 && port < 2440) {
                i++;
                final byte[] ipSizeAsBytes = new byte[] { buffer[++i], buffer[++i], buffer[++i], buffer[++i] };
                ...
                if (ipSize > 0 && ipSize <= 15) {
                  ...
                  if (parts.length == 4) {
                    ...
                    if (!invalid) {
                      ...
                      OLogManager.instance().info(this, "ProxyChannel: patching port %d to %d", port, port + 1000);
                    }
                  }
                }
              }
    ...
    Orient.instance().addDbLifecycleListener(new ODatabaseLifecycleListenerAbstract() {
      @Override
