package com.orientechnologies.orient.test.database.speed;

import org.testng.annotations.Test;

@Test(sequential = true)
public class FullTextSearchTest {
  // private static final int DOCUMENTS = 1000;

  public static void main(String[] iArgs) throws InstantiationException, IllegalAccessException {
    // OProfilerStub.getInstance().startRecording();
    //
    // final ODatabaseDocumentTx database = new
    // ODatabaseDocumentTx(System.getProperty("url")).open("admin", "admin");
    //
    // database.declareIntent(new OIntentMassiveRead());
    // database.begin(TXTYPE.NOTX);
    //
    // long time = System.currentTimeMillis();
    //
    // List<ORecordId> recs = null;// index.get("wife");
    //
    // if (recs == null)
    // System.out.println("\nERROR: Not found!");
    // else
    // System.out.println("\nSearch for keyword 'wife' in " + ((System.currentTimeMillis() - time) /
    // 1000f) + " sec.");

    // for (Entry<String, List<ORecordId>> entry : index) {
    // System.out.println("- " + entry.getKey() + ":" + entry.getValue().size());
    // }

    // database.close();
  }
}
