...
public class SQLCreateLinkTest extends DocumentDBBaseTest {
  ...
  public void createLinktest() {
    ...
    Assert
        .assertTrue(database.command(new OCommandSQL("INSERT INTO POST (id, title) VALUES ( 30, '(')")).execute() instanceof ODocument);

    Assert
        .assertTrue(database.command(new OCommandSQL("INSERT INTO POST (id, title) VALUES ( 40, ')')")).execute() instanceof ODocument);
    ...
    Assert.assertEquals(
        ((Number) database.command(new OCommandSQL("CREATE LINK comments TYPE LINKSET FROM comment.postId TO post.id INVERSE"))
            .execute()).intValue(), 5);
  ...
  public void createRIDLinktest() {
    ...
    Assert.assertTrue(database.command(
        new OCommandSQL("INSERT INTO COMMENT2 (id, postId, text) VALUES ( 0, '" + ((ODocument) p1).getIdentity().toString()
            + "', 'First' )")).execute() instanceof ODocument);
    Assert.assertTrue(database.command(
        new OCommandSQL("INSERT INTO COMMENT2 (id, postId, text) VALUES ( 1, '" + ((ODocument) p1).getIdentity().toString()
            + "', 'Second' )")).execute() instanceof ODocument);
    Assert.assertTrue(database.command(
        new OCommandSQL("INSERT INTO COMMENT2 (id, postId, text) VALUES ( 21, '" + ((ODocument) p1).getIdentity().toString()
            + "', 'Another' )")).execute() instanceof ODocument);
    Assert.assertTrue(database.command(
        new OCommandSQL("INSERT INTO COMMENT2 (id, postId, text) VALUES ( 41, '" + ((ODocument) p2).getIdentity().toString()
            + "', 'First again' )")).execute() instanceof ODocument);
    Assert.assertTrue(database.command(
        new OCommandSQL("INSERT INTO COMMENT2 (id, postId, text) VALUES ( 82, '" + ((ODocument) p2).getIdentity().toString()
            + "', 'Second Again' )")).execute() instanceof ODocument);
