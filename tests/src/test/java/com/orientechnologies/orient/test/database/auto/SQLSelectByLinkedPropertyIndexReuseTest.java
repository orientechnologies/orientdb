...
public class SQLSelectByLinkedPropertyIndexReuseTest extends AbstractIndexReuseTest {
  ...
  public void testNotUniqueUniqueNotUniqueEqualsUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.name = 'Someone'"));
  ...
  public void testNotUniqueUniqueUniqueEqualsUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.salary = 600"));
  ...
  public void testNotUniqueUniqueNotUniqueEqualsLimitUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.name = 'Someone else' limit 1"));
    assertEquals(result.size(), 1);
  ...
  public void testNotUniqueUniqueUniqueMinorUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.salary < 1000"));
  ...
  public void testNotUniqueUniqueUniqueMinorLimitUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.salary < 1000 limit 2"));
  ...
  public void testNotUniqueUniqueUniqueMajorLimitUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where group.curator.salary > 550 limit 1"));
    assertEquals(result.size(), 1);
  ...
  public void testUniqueUniqueBetweenUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtGroup where curator.salary between 500 and 1000"));
    assertEquals(result.size(), 2);
  ...
  public void testUniqueUniqueBetweenLimitUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtGroup where curator.salary between 500 and 1000 limit 1"));
    assertEquals(result.size(), 1);
  ...
  public void testUniqueUniqueInUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtGroup where curator.salary in [500, 600]"));
    assertEquals(result.size(), 2);
    assertEquals(containsDocumentWithFieldValue(result, "name", "PZ-08-2"), 1);
    assertEquals(containsDocumentWithFieldValue(result, "name", "PZ-08-3"), 1);
  ...
  public void testUniqueUniqueInLimitUsing() throws Exception {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtGroup where curator.salary in [500, 600] limit 1"));
    assertEquals(result.size(), 1);
  ...
  public void testUniquePartialSearch() {
    ...
    List<ODocument> result = database
        .query(new OSQLSynchQuery<ODocument>("select from lpirtStudent where diploma.name = 'diploma3'"));
  ...
  private void fillDataSet() {
    ...
    diploma3.field("thesis",
        "College student shouldn't have to take a required core curriculum, and many core " + "courses are graded too stiffly.");
  ...
  private void createSchemaForTest() {
    ...
    if (!schema.existsClass("lpirtStudent")) {
      ...
      curatorClass.createProperty("salary", OType.INTEGER)
          .createIndex(OClass.INDEX_TYPE.UNIQUE, new ODocument().field("ignoreNullValues", true));
      curatorClass.createIndex("curotorCompositeIndex", OClass.INDEX_TYPE.UNIQUE.name(), null,
      ...
      groupClass.createProperty("name", OType.STRING)
          .createIndex(OClass.INDEX_TYPE.UNIQUE, new ODocument().field("ignoreNullValues", true));
      groupClass.createProperty("curator", OType.LINK, curatorClass)
          .createIndex(OClass.INDEX_TYPE.UNIQUE, new ODocument().field("ignoreNullValues", true));
      ...
      diplomaClass.createProperty("name", OType.STRING)
          .createIndex(OClass.INDEX_TYPE.UNIQUE, new ODocument().field("ignoreNullValues", true));
      diplomaClass.createIndex("diplomaThesisUnique", OClass.INDEX_TYPE.UNIQUE.name(), null,
      ...
      transcriptClass.createProperty("id", OType.STRING)
          .createIndex(OClass.INDEX_TYPE.UNIQUE_HASH_INDEX, new ODocument().field("ignoreNullValues", true));

      final OClass skillClass = schema.createClass("lpirtSkill");
      skillClass.createProperty("name", OType.STRING)
          .createIndex(OClass.INDEX_TYPE.UNIQUE, new ODocument().field("ignoreNullValues", true));
      ...
      studentClass.createProperty("transcript", OType.LINK, transcriptClass)
          .createIndex(OClass.INDEX_TYPE.UNIQUE_HASH_INDEX, new ODocument().field("ignoreNullValues", true));
      ...
      studentClass
          .createIndex("studentSkillAndGroupIndex", OClass.INDEX_TYPE.NOTUNIQUE_HASH_INDEX.toString(), null, metadata.copy(),
