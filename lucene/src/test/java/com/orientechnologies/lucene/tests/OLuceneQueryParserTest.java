...
public class OLuceneQueryParserTest extends OLuceneBaseTest {
  ...
  public void shouldSearchWithLowercaseExpandedTerms() {
    ...
    //enabling leading wildcard
    db.command("create index Song.author on Song (author) FULLTEXT ENGINE LUCENE metadata {\"default\": \"" + KeywordAnalyzer.class
  ...
  public void shouldUseBoostsFromQuery() throws Exception {
    ...
    assertThat(boostedDocs)
        .contains("THIS TIME FOREVER", "FOREVER YOUNG", "TOMORROW IS FOREVER", "STARS AND STRIPES FOREVER" //boosted
    ...
    assertThat(docs).contains("THIS TIME FOREVER", "FOREVER YOUNG", "TOMORROW IS FOREVER", "ALL I HAVE TO DO IS DREAM",
  ...
  public void shouldUseBoostsFromMapAndSyntax() throws Exception {
    ...
    List<Float> boostedDocs = rs.stream().map(r -> r.<Float>getProperty("$score")).collect(Collectors.toList());

    assertThat(boostedDocs).hasSize(5);

    rs.close();

    rs = db.query("select $score from Song where search_class ('(title:forever)^2 OR author:Boudleaux')=true order by $score desc");
    List<Float> docs = rs.stream().map(r -> r.<Float>getProperty("$score")).collect(Collectors.toList());
  ...
  public void ahouldOverrideAnalyzer() throws Exception {
    ...
    OResultSet resultSet = db.query(
        "select * from Song where search_class ('title:forever OR author:boudleaux' , " + "{'customAnalysis': true, "
            + "  \"query\": \"org.apache.lucene.analysis.core.KeywordAnalyzer\" } " + ")=true");
