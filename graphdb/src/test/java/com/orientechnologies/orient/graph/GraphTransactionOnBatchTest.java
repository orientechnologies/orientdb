...
public class GraphTransactionOnBatchTest {
  ...
  public void testAbsoluteDuplicateEdgeAlreadyPresentRollback() {
    ...
    try {
      ...
      db.execute(
              "sql",
              "BEGIN \n"
                  + "; LET c =create edge Test from (select from V  where name='a') to (select from"
                  + " V where name='b')  \n"
                  + "; COMMIT \n"
                  + "; RETURN $c")
          .close();
  ...
  /**
   * This test is different from the original reported, because in case of empty query result the
   * 'create edge ' command just don't create edges without failing
   */
  @Test
