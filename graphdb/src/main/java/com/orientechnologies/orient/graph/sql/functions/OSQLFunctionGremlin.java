...
public class OSQLFunctionGremlin extends OSQLFunctionAbstract {
  ...
  public Object execute(
    ...
    final ODatabaseDocumentInternal db =
        OGremlinHelper.getGraphDatabase(ODatabaseRecordThreadLocal.instance().get());
        ...
        new OGremlinHelper.OGremlinCallback() {
          ...
          public boolean call(final ScriptEngine iEngine, final OrientBaseGraph iGraph) {
            ...
            if (clazz != null && clazz.isSubClassOf(OrientEdgeType.CLASS_NAME)) {
              ...
              OrientEdge graphElement =
                  (OrientEdge)
                      new OrientElementIterable<OrientEdge>(
                              iGraph, Arrays.asList(new ODocument[] {document}))
                          .iterator()
                          .next();
              iEngine.getBindings(ScriptContext.ENGINE_SCOPE).put("current", graphElement);
              iEngine
                  .getBindings(ScriptContext.ENGINE_SCOPE)
                  .put("it", graphElement); // FRAMES LIKE SYNTAX
              ...
              OrientVertex graphElement =
                  (OrientVertex)
                      new OrientElementIterable<OrientVertex>(
                              iGraph, Arrays.asList(new ODocument[] {document}))
                          .iterator()
                          .next();
              iEngine.getBindings(ScriptContext.ENGINE_SCOPE).put("current", graphElement);
