...
public class OGremlinHelper {
   ...
   public static Object execute(final OGraphDatabase iDatabase, final String iText, final Map<Object, Object> iArgs,
         final List<Object> iResult, final OGremlinCallback iBeforeExecution, final OGremlinCallback iAfterExecution) {
      ...
      try {
         ...
         try {
            ...
            final String output = iArgs != null ? OGremlinHelper.bindParameters(engine, iArgs) : null;
...
//                     current = ((OrientElement) current).getRawElement();
   ...
   public static String bindParameters(final GremlinScriptEngine iEngine, final Map<Object, Object> iParameters) {
      String output = null;
      final Map<Object, Object> params = iParameters;
      if (params != null) {
         for (Entry<Object, Object> param : params.entrySet()) {
            if (param.getKey().toString().trim().equals(PARAM_OUTPUT)) {
               output = param.getValue().toString();
               continue;
            }
            iEngine.getBindings(ScriptContext.ENGINE_SCOPE).put(param.getKey().toString(), param.getValue());
         }
      }
      return output;
   }
