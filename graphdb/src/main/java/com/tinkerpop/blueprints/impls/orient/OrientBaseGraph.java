...
public abstract class OrientBaseGraph extends OrientConfigurableGraph implements OrientExtendedGraph {
   ...
   */
  public OrientBaseGraph(final ODatabaseDocumentTx iDatabase, final String iUserName, final String iUserPassword, final Settings iConfiguration) {
    this.pool = null;
  ...
  public <T extends Element> void createKeyIndex(final String key, final Class<T> elementClass, final Parameter... indexParameters) {
    ...
    executeOutsideTx(new OCallable<OClass, OrientBaseGraph>() {
      ...
      public OClass call(final OrientBaseGraph g) {
        ...
        db.getMetadata().getIndexManager().createIndex(className + "." + key, indexType, new OPropertyIndexDefinition(className, key, keyType), cls.getPolymorphicClusterIds(), null, metadata);
        return null;
