    ...
    new Thread(() -> {
      ...
      try {
        ...
        for (int i = 0; i < BRUTE_FORCE_ATTEMPT; i++) {
          try (ODatabaseSession session = context.open(BruteForceSecurityTest.class.getSimpleName(), "fake", "fake")) {
            Assert.fail();
    ...
    new Thread(() -> {
      ...
      try {
        ...
        for (int i = 0; i < BRUTE_FORCE_ATTEMPT / 100; i++) {
          try (ODatabaseSession session = context.open(BruteForceSecurityTest.class.getSimpleName(), "admin", "fake" + i)) {
            Assert.fail();
          } catch (OSecurityAccessException e) {
          }
        }
    ...
    new Thread(() -> {
      ...
      try {
        for (int i = 0; i < BRUTE_FORCE_ATTEMPT; i++) {
          ...
          long end;
          try (ODatabaseSession open = context.open(BruteForceSecurityTest.class.getSimpleName(), "admin", "admin")) {
