...
/**
 * SQL HA STOP REPLICATION command: stops the replication against a server. The command is sent to all the servers, but executed
 * only on the server where the replication will start.
 * 
 * @author Luca Garulli
 * 
 */
@SuppressWarnings("unchecked")
public class OCommandExecutorSQLHAStopReplication extends OCommandExecutorSQLAbstract
    implements OCommandDistributedReplicateRequest {
  public static final String NAME                = "HA STOP REPLICATION";
  ...
  public OCommandExecutorSQLHAStopReplication parse(final OCommandRequest iRequest) {
    ...
    if (pos == -1 || !word.toString().equals(KEYWORD_STOP))
      throw new OCommandSQLParsingException("Keyword " + KEYWORD_STOP + " not found. Use " + getSyntax(), parserText, oldPos);
    ...
    if (!(stg instanceof ODistributedStorage))
      throw new ODistributedException("HA STOP REPLICATION command cannot be executed against a non distributed server");
  ...
  public Object execute(final Map<Object, Object> iArgs) {
    ...
    if (!server.equalsIgnoreCase(dManager.getLocalNodeName())) {
      return true;
    }
    ...
    if (dropDatabase) {
      ...
      dManager.setDatabaseStatus(dManager.getLocalNodeName(), database.getName(), ODistributedServerManager.DB_STATUS.OFFLINE);
  ...
  public String getSyntax() {
    return "HA STOP REPLICATION <server> [-dropDatabase]";
  }
}
