...
public enum OGlobalMetrics {
  SERVER_DATABASES("server.info.databases", "List of databases configured in Server"), SERVER_NETWORK_REQUESTS(
      ...
      "Stats about Disk Cache"), SERVER_DISK_SPACE("server.disk.space", "Stats about disk space"), DATABASE_READ_OPS(
      "db.%s.readOps", "Stats on read operations"), DATABASE_CREATE_OPS("db.%s.createOps",
      "Stats on create operations"), DATABASE_UPDATE_OPS("db.%s.updateOps", "Stats on update operations"), DATABASE_DELETE_OPS(
