...
public enum OGlobalMetrics {
  SERVER_DATABASES("server.info.databases", "List of databases configured in Server"), SERVER_NETWORK_REQUESTS(
      ...
      "Stats about Disk Cache"), DATABASE_READ_OPS("db.%s.readOps", "Stats on read operations"), DATABASE_CREATE_OPS(
      "db.%s.createOps", "Stats on create operations"), DATABASE_UPDATE_OPS("db.%s.updateOps",
      "Stats on update operations"), DATABASE_DELETE_OPS("db.%s.deleteOps", "Stats on delete operations"), DATABASE_COMMIT_OPS(
      "db.%s.commitOps", "Stats on commit operations"), DATABASE_ROLLBACK_OPS("db.%s.rollbackOps", "Stats on rollback operations");
