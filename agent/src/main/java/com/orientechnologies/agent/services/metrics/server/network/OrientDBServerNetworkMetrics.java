...
public class OrientDBServerNetworkMetrics implements OrientDBMetric {
  ...
  public void start() {
    ...
    this.networkRequests =
        this.registry.meter(
            OGlobalMetrics.SERVER_NETWORK_REQUESTS.name,
            OGlobalMetrics.SERVER_NETWORK_REQUESTS.description,
            "Events/seconds");

    server.registerConnectionListener(
        new OEnterpriseConnectionListener() {
    ...
    this.registry.gauge(
        OGlobalMetrics.SERVER_NETWORK_SESSIONS.name,
        OGlobalMetrics.SERVER_NETWORK_SESSIONS.description,
        () -> connections.get());

    this.registry.gauge(
        OGlobalMetrics.SERVER_NETWORK_SOCKETS.name,
        OGlobalMetrics.SERVER_NETWORK_SOCKETS.description,
        () -> sockets.get());
  }
