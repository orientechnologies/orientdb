...
uint64_t crc32(const uint8_t *data, uint32_t  size) {
    uint32_t crc = 0xffffffff;

    while(size > 3) {
          uint32_t c0 = crc ^ *data++;
          uint32_t c1 = (crc >>= 8) ^ *data++;
          uint32_t c2 = (crc >>= 8) ^ *data++;
          uint32_t c3 = (crc >>= 8) ^ *data++;
