...
public class IssueRepositoryImpl extends OrientBaseRepository<Issue> implements IssueRepository {
    ...
    public List<OUser> findInvolvedActors(Issue issue) {
        ...
        String query = String.format("select  expand(set(user)) from (select unionAll( in('HasOpened'),out('HasEvent')[@class = 'Comment'].user,in('HasIssue').in('HasRepo').out('HasRepo').out('HasMember')) as user from %s )", rid);
