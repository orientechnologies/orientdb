	...
	public static void setUpClass () {
		try {
			PropertyConfigurator.configure(new File(
					OrientGraphJdbcBaseTest.class.getClassLoader().getResource("log4j.properties").toURI()).getAbsolutePath());
	...
	public void setUp () {
		try {
			conn = (OrientJdbcConnection) 
				DriverManager.getConnection(DB_URL_PREFIX + OrientGraphJdbcCreationHelper.URL_DB,info);
