...
public class OrientBlob implements Blob {
	...
	private List<byte[]> binaryDataChunks;
	...
	protected OrientBlob (List<ORecordBytes> binaryDataChunks)
	...
	protected OrientBlob (ORecordBytes binaryDataChunk)
	throws IllegalArgumentException, NullPointerException {
		if (binaryDataChunk.getSize() == 0)
			throw new IllegalArgumentException ("The binary data chunks list cannot hold empty chunks");
		this.binaryDataChunks = new ArrayList<byte[]> ();
		this.binaryDataChunks.add(binaryDataChunk.toStream());
		this.length = calculateLenght();
	}
	...
	public byte[] getBytes(long pos, int length) throws SQLException {
		if (pos < 1)
			throw new SQLException ("The position of the first byte in the BLOB value to be " +
		...
		for (j = 0; j < length; j++) {
			if (relativeIndex == currentChunk.length) {
				...
				currentChunkIndex++;
				if (currentChunkIndex < binaryDataChunks.size() - 1) {
					//the next chunk exists so we update the relative index and the current chunk reference
					relativeIndex = 0;
	...
	private int getRelativeIndex (long pos) {
		...
		System.out.println(binaryDataChunks.get(currentChunkIndex).length);
	...
	private class OrientBlobInputStream extends InputStream {
		...
		public int read() throws IOException {
			if (bytesToBeRead > 0) {
				...
				//if all the bytes in the current binary chunk have been read, we move to the next one
				if (positionInTheCurrentChunk == OrientBlob.this.currentChunk.length - 1) {
					...
					if (OrientBlob.this.currentChunkIndex == OrientBlob.this.binaryDataChunks.size() - 1) {
						...
						OrientBlob.this.currentChunk = OrientBlob.this.binaryDataChunks.get(
							++OrientBlob.this.currentChunkIndex);
						positionInTheCurrentChunk = 0;
					}
				}
				bytesToBeRead--;
